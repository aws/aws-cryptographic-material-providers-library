...
class default__:
    ...
    def SequenceEqualNat(seq1, seq2, start1, start2, size):
        if ((len(seq1)) > (StandardLibrary_UInt.default__.UINT64__MAX__LIMIT)) or ((len(seq2)) > (StandardLibrary_UInt.default__.UINT64__MAX__LIMIT)):
            return (_dafny.Seq((seq1)[start1:(start1) + (size):])) == (_dafny.Seq((seq2)[start2:(start2) + (size):]))
        elif True:
