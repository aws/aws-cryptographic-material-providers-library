...
class default__:
    ...
    def LiftCursorError(err):
        ...
        if True:
            if source0_.is_ExpectingByte:
                d_0_expected_ = source0_.expected
                d_1_b_ = source0_.b
                return JSON_Errors.DeserializationError_ExpectingByte(d_0_expected_, d_1_b_)
        if True:
            if source0_.is_ExpectingAnyByte:
                d_2_expected__sq_ = source0_.expected__sq
                d_3_b_ = source0_.b
                return JSON_Errors.DeserializationError_ExpectingAnyByte(d_2_expected__sq_, d_3_b_)
        if True:
            d_4_err_ = source0_.err
            return d_4_err_
    ...
    @staticmethod
    def Text(v):
        d_0_valueOrError0_ = default__.JSON(JSON_Utils_Cursors.Cursor__.OfView(v))
        if (d_0_valueOrError0_).IsFailure():
            return (d_0_valueOrError0_).PropagateFailure()
        elif True:
            let_tmp_rhs0_ = (d_0_valueOrError0_).Extract()
            d_1_text_ = let_tmp_rhs0_.t
            d_2_cs_ = let_tmp_rhs0_.cs
            d_3_valueOrError1_ = Wrappers.default__.Need((d_2_cs_).EOF_q, JSON_Errors.DeserializationError_ExpectingEOF())
            if (d_3_valueOrError1_).IsFailure():
                return (d_3_valueOrError1_).PropagateFailure()
            elif True:
                return Wrappers.Result_Success(d_1_text_)

    @staticmethod
    def OfBytes(bs):
        d_0_valueOrError0_ = Wrappers.default__.Need((len(bs)) < (BoundedInts.default__.TWO__TO__THE__32), JSON_Errors.DeserializationError_IntOverflow())
        if (d_0_valueOrError0_).IsFailure():
            return (d_0_valueOrError0_).PropagateFailure()
        elif True:
