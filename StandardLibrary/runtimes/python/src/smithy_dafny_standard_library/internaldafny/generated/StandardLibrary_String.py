...
class default__:
    ...
    def HasSubString(haystack, needle):
        ...
        if (len(haystack)) < (len(needle)):
            ...
            return o
        if not((len(haystack)) <= ((StandardLibrary_UInt.default__.UINT64__MAX__LIMIT) - (1))):
            raise _dafny.HaltException("src/String.dfy(75,4): " + _dafny.string_of(_dafny.Seq("expectation violation")))
        d_0_size_: int
        d_0_size_ = len(needle)
        d_1_limit_: int
        d_1_limit_ = ((len(haystack)) - (d_0_size_)) + (1)
        hi0_ = d_1_limit_
