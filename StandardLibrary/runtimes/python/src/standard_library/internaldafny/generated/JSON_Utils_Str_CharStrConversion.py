...
class default__:
    ...
    def Digits(n, base):
        if (n) == (0):
            ...
            d_466_digits_k_ = default__.Digits(_dafny.euclidian_division(n, base), base)
            d_467_digits_ = (d_466_digits_k_) + (_dafny.Seq([_dafny.euclidian_modulus(n, base)]))
            return d_467_digits_

    @staticmethod
    def OfDigits(digits, chars):
        d_468___accumulator_ = _dafny.Seq([])
        while True:
            with _dafny.label():
                if (digits) == (_dafny.Seq([])):
                    return (d_468___accumulator_) + (_dafny.Seq([]))
                elif True:
                    d_468___accumulator_ = (d_468___accumulator_) + (_dafny.Seq([(chars)[(digits)[0]]]))
    ...
    def OfNat__any(n, chars):
        d_469_base_ = len(chars)
        if (n) == (0):
            return _dafny.Seq([(chars)[0]])
        elif True:
            return default__.OfDigits(default__.Digits(n, d_469_base_), chars)

    @staticmethod
    def NumberStr(str, minus, is__digit):
        def lambda29_(forall_var_7_):
            d_470_c_: str = forall_var_7_
            return not ((d_470_c_) in (_dafny.Seq((str)[1::]))) or (is__digit(d_470_c_))
