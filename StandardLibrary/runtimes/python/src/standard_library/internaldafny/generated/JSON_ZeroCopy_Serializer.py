    ...
    def Value(v, writer):
        ...
        if True:
            if source19_.is_Null:
                ...
                return (writer).Append(d_656_n_)
        if True:
            if source19_.is_Bool:
                d_657_b_ = source19_.b
                d_658_wr_ = (writer).Append(d_657_b_)
                return d_658_wr_
        if True:
            if source19_.is_String:
                d_659_str_ = source19_.str
                d_660_wr_ = default__.String(d_659_str_, writer)
                return d_660_wr_
        if True:
            if source19_.is_Number:
                d_661_num_ = source19_.num
                d_662_wr_ = default__.Number(d_661_num_, writer)
                return d_662_wr_
        if True:
            if source19_.is_Object:
                d_663_obj_ = source19_.obj
                d_664_wr_ = default__.Object(d_663_obj_, writer)
                return d_664_wr_
        if True:
            d_665_arr_ = source19_.arr
            d_666_wr_ = default__.Array(d_665_arr_, writer)
            return d_666_wr_
