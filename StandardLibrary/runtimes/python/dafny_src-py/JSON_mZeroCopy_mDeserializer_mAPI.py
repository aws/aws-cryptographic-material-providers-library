...
# Module: JSON_mZeroCopy_mDeserializer_mAPI
...
class default__:
    ...
    @staticmethod
    def JSON(cs):
        return (JSON_mZeroCopy_mDeserializer_mCore.default__.Structural(cs, JSON_mUtils_mParsers.Parser___Parser(JSON_mZeroCopy_mDeserializer_mValues.default__.Value))).MapFailure(default__.LiftCursorError)

    @staticmethod
    def Text(v):
        d_802_valueOrError0_ = default__.JSON(JSON_mUtils_mCursors.Cursor__.OfView(v))
    ...
    def OfBytes(bs):
        ...
        if (d_806_valueOrError0_).IsFailure():
            ...
            return default__.Text(JSON_mUtils_mViews_mCore.View__.OfBytes(bs))
