...
# Module: JSON_mZeroCopy_mDeserializer_mObjects
...
class default__:
        ...
        while True:
            with _dafny.label():
                ...
                if (d_717_valueOrError0_).IsFailure():
                    ...
                    if (d_720_s0_) == (default__.SEPARATOR):
                        d_721_elems_ = default__.AppendWithSuffix(elems, d_718_elem_, d_719_sep_)
                        in123_ = json
                        in124_ = open
                        in125_ = d_721_elems_
                        json = in123_
                        ...
                        d_722_elems_k_ = default__.AppendLast(elems, d_718_elem_, d_719_sep_)
                        d_723_bracketed_ = default__.BracketedFromParts(open, d_722_elems_k_, d_719_sep_)
                        return Wrappers.Result_Success(d_723_bracketed_)
                    elif True:
                        d_724_separator_ = default__.SEPARATOR
    ...
    def Bracketed(cs, json):
        d_726_valueOrError0_ = JSON_mZeroCopy_mDeserializer_mCore.default__.Structural(cs, JSON_mUtils_mParsers.Parser___Parser(default__.Open))
        if (d_726_valueOrError0_).IsFailure():
            ...
            if (((d_727_open_).cs).Peek()) == (JSON_mZeroCopy_mDeserializer_mObjectParams.default__.CLOSE):
                d_729_valueOrError1_ = JSON_mZeroCopy_mDeserializer_mCore.default__.Structural((d_727_open_).cs, JSON_mUtils_mParsers.Parser___Parser(default__.Close))
                if (d_729_valueOrError1_).IsFailure():
                    return (d_729_valueOrError1_).PropagateFailure()
                elif True:
                    d_730_close_ = (d_729_valueOrError1_).Extract()
                    return Wrappers.Result_Success(default__.BracketedFromParts(d_727_open_, d_728_elems_, d_730_close_))
            elif True:
                return default__.Elements(json, d_727_open_, d_728_elems_)
