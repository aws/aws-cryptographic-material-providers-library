...
class default__:
    ...
    def IsHexString(s):
        def lambda8_(forall_var_3_):
            ...
            return not ((d_134_ch_) in (s)) or (default__.IsHexChar(d_134_ch_))
    ...
    def IsLooseHexString(s):
        def lambda9_(forall_var_4_):
            ...
            return not ((d_135_ch_) in (s)) or (default__.IsLooseHexChar(d_135_ch_))
    ...
    def HexStr(x):
        if (x) < (16):
            d_136_res_ = _dafny.Seq(['0', default__.HexChar(x)])
            return d_136_res_
        elif True:
            d_137_res_ = _dafny.Seq([default__.HexChar(_dafny.euclidian_division(x, 16)), default__.HexChar(_dafny.euclidian_modulus(x, 16))])
            return d_137_res_
        ...
        while True:
            with _dafny.label():
                if (len(val)) == (0):
                    ...
                    d_138___accumulator_ = (d_138___accumulator_) + (default__.HexStr((val)[0]))
        ...
        while True:
            with _dafny.label():
                if (len(data)) == (0):
                    ...
                    d_139___accumulator_ = (d_139___accumulator_) + (_dafny.Seq([default__.HexVal((data)[0])]))
                    in25_ = _dafny.Seq((data)[1::])
                    data = in25_
                    raise _dafny.TailCall()
                elif True:
                    d_139___accumulator_ = (d_139___accumulator_) + (_dafny.Seq([default__.HexValue(_dafny.Seq((data)[:2:]))]))
