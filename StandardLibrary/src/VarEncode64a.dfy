...
module {:options "-functionSyntax:4"} VarEncode64a {
  ...
  function Gather(x : seq<uint8>, acc : uint64) : uint64
  {
    if |x| == 0 then
      acc
    else
      var newVal := (acc as int * 256) + x[0] as int;
      assume {:axiom} newVal as int < UINT64_LIMIT;
      Gather(x[1..], newVal as uint64)
  }
