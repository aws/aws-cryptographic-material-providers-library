...
module {:options "-functionSyntax:4"} VarEncode {
  ...
  */
  ...
  //   match len {
  //     case 1 => x[0] < Tag2
  //     case 2 => Tag2 < x[0] || (x[0] == Tag2 && x[1] >= 128)
  //     case 3 => Tag3 < x[0] || (x[0] == Tag3 && x[1] >= 64)
  //     case 4 => Tag4 < x[0] || (x[0] == Tag4 && x[1] >= 32)
  //     case 5 => Tag5 < x[0] || (x[0] == Tag5 && x[1] >= 16)
  //     case 6 => Tag6 < x[0] || (x[0] == Tag6 && x[1] >= 8)
  //     case 7 => Tag7 < x[0] || (x[0] == Tag7 && x[1] >= 4)
  //     case 8 => x[0] == 0xfe && x[1] >= 2
  //     case 9 => x[0] == 0xff && 0 < x[1] < Tag2
  //     case 10 => x[0] == 0xff && x[1] == Tag2 && Tag2 <= x[2]
  //   }
  ...
  //   ensures DecodeLength?(s).Success?
  //   ensures DecodeLength?(s).value == DecodeLength(s) as nat
  // {
  //   assert DecodeLength?(s).Success?;
  //   assert DecodeLength?(s).value == 11;
  ...
  lemma {:vcs_split_on_every_assert} {:only} Foo(x: uint64, s: seq<uint8>, i: nat)
    requires 0 < i < |s| == 8
    requires DecodeLength(s) == |s| as MyLen
    requires ValidEncoding(s, |s| as MyLen)
    ...
    decreases 8 - i
  {
    if i == 7 {
      ...
      assert Pow(7-i) == 0x1;
      assert s[i] == ((x as nat % Pow(8-i)) / Pow(7-i)) as uint8;
    } else {
      assume i == 6;
      assert Pow(8-i) == 0x10000;
      assert Pow(7-i) == 0x100;
      assert s[i] == ((x as nat % Pow(8-i)) / Pow(7-i)) as uint8;
    }
  }

  lemma {:only} asdfsss(s: seq<uint8>)
