...
module {:options "-functionSyntax:4"} VarEncode64 {
    ...
    ensures ret[9] == (x % 0x100) as uint8
  {
    ...
    var byte7:= ((x % 0x10000) / 0x100) as uint8;
    var byte8:= (x % 0x100) as uint8;
    [Tag9, Tag2, byte1, byte2, byte3, byte4, byte5, byte6, byte7, byte8]
  }
    ...
    ensures Encode(x) == s
  {
    ...
    assume {:axiom} s[4] == ((x % 0x1000000) / 0x10000) as uint8;
    ...
    ensures Encode(x) == s
  {
    ...
    assume {:axiom} s[5] == ((x % 0x1000000) / 0x10000) as uint8;
    assume {:axiom} s[6] ==((x % 0x10000) / 0x100) as uint8;
    ...
    ensures Encode(x) == s
  {
    match len {
      ...
      case 5 => assume {:axiom} Encode5(x) == s;
      ...
      case 10 => assume {:axiom} Encode10(x) == s;
    }
  }
