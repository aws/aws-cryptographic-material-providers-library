...
def com_amazonaws_kms_XksProxyVpcEndpointServiceInvalidConfigurationException(native_input):
...
def com_amazonaws_kms_CancelKeyDeletionRequest(native_input):
    return DafnyCancelKeyDeletionRequest(
        KeyId=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyId"].encode('utf-16-be'))]*2)])),
    )

def com_amazonaws_kms_CancelKeyDeletionResponse(native_input):
    return DafnyCancelKeyDeletionResponse(
        KeyId=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyId"].encode('utf-16-be'))]*2)]))) if "KeyId" in native_input.keys() else Option_None(),
    )

def com_amazonaws_kms_ConnectCustomKeyStoreRequest(native_input):
    return DafnyConnectCustomKeyStoreRequest(
        CustomKeyStoreId=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["CustomKeyStoreId"].encode('utf-16-be'))]*2)])),
    )
...
def com_amazonaws_kms_CreateAliasRequest(native_input):
    return DafnyCreateAliasRequest(
        AliasName=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["AliasName"].encode('utf-16-be'))]*2)])),
        TargetKeyId=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["TargetKeyId"].encode('utf-16-be'))]*2)])),
    )

def com_amazonaws_kms_CreateCustomKeyStoreRequest(native_input):
    return DafnyCreateCustomKeyStoreRequest(
        CustomKeyStoreName=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["CustomKeyStoreName"].encode('utf-16-be'))]*2)])),
        CloudHsmClusterId=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["CloudHsmClusterId"].encode('utf-16-be'))]*2)]))) if "CloudHsmClusterId" in native_input.keys() else Option_None(),
        TrustAnchorCertificate=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["TrustAnchorCertificate"].encode('utf-16-be'))]*2)]))) if "TrustAnchorCertificate" in native_input.keys() else Option_None(),
        KeyStorePassword=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyStorePassword"].encode('utf-16-be'))]*2)]))) if "KeyStorePassword" in native_input.keys() else Option_None(),
        CustomKeyStoreType=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_CustomKeyStoreType(native_input["CustomKeyStoreType"])) if "CustomKeyStoreType" in native_input.keys() else Option_None(),
        XksProxyUriEndpoint=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["XksProxyUriEndpoint"].encode('utf-16-be'))]*2)]))) if "XksProxyUriEndpoint" in native_input.keys() else Option_None(),
        XksProxyUriPath=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["XksProxyUriPath"].encode('utf-16-be'))]*2)]))) if "XksProxyUriPath" in native_input.keys() else Option_None(),
        XksProxyVpcEndpointServiceName=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["XksProxyVpcEndpointServiceName"].encode('utf-16-be'))]*2)]))) if "XksProxyVpcEndpointServiceName" in native_input.keys() else Option_None(),
        XksProxyAuthenticationCredential=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_XksProxyAuthenticationCredentialType(native_input["XksProxyAuthenticationCredential"])) if "XksProxyAuthenticationCredential" in native_input.keys() else Option_None(),
        XksProxyConnectivity=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_XksProxyConnectivityType(native_input["XksProxyConnectivity"])) if "XksProxyConnectivity" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_XksProxyAuthenticationCredentialType(native_input):
    return DafnyXksProxyAuthenticationCredentialType(
        AccessKeyId=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["AccessKeyId"].encode('utf-16-be'))]*2)])),
        RawSecretAccessKey=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["RawSecretAccessKey"].encode('utf-16-be'))]*2)])),
    )
...
def com_amazonaws_kms_CreateCustomKeyStoreResponse(native_input):
    return DafnyCreateCustomKeyStoreResponse(
        CustomKeyStoreId=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["CustomKeyStoreId"].encode('utf-16-be'))]*2)]))) if "CustomKeyStoreId" in native_input.keys() else Option_None(),
    )

def com_amazonaws_kms_CreateGrantRequest(native_input):
    return DafnyCreateGrantRequest(
        KeyId=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyId"].encode('utf-16-be'))]*2)])),
        GranteePrincipal=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["GranteePrincipal"].encode('utf-16-be'))]*2)])),
        RetiringPrincipal=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["RetiringPrincipal"].encode('utf-16-be'))]*2)]))) if "RetiringPrincipal" in native_input.keys() else Option_None(),
        Operations=Seq([aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_GrantOperation(list_element) for list_element in native_input["Operations"]]),
        Constraints=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_GrantConstraints(native_input["Constraints"])) if "Constraints" in native_input.keys() else Option_None(),
        GrantTokens=Option_Some(Seq([Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(list_element.encode('utf-16-be'))]*2)])) for list_element in native_input["GrantTokens"]])) if "GrantTokens" in native_input.keys() else Option_None(),
        Name=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["Name"].encode('utf-16-be'))]*2)]))) if "Name" in native_input.keys() else Option_None(),
        DryRun=Option_Some(native_input["DryRun"]) if "DryRun" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_GrantConstraints(native_input):
    return DafnyGrantConstraints(
        EncryptionContextSubset=Option_Some(Map({Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(key.encode('utf-16-be'))]*2)])): Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(value.encode('utf-16-be'))]*2)])) for (key, value) in native_input["EncryptionContextSubset"].items() })) if "EncryptionContextSubset" in native_input.keys() else Option_None(),
        EncryptionContextEquals=Option_Some(Map({Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(key.encode('utf-16-be'))]*2)])): Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(value.encode('utf-16-be'))]*2)])) for (key, value) in native_input["EncryptionContextEquals"].items() })) if "EncryptionContextEquals" in native_input.keys() else Option_None(),
    )

def com_amazonaws_kms_CreateGrantResponse(native_input):
    return DafnyCreateGrantResponse(
        GrantToken=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["GrantToken"].encode('utf-16-be'))]*2)]))) if "GrantToken" in native_input.keys() else Option_None(),
        GrantId=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["GrantId"].encode('utf-16-be'))]*2)]))) if "GrantId" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_CreateKeyRequest(native_input):
    return DafnyCreateKeyRequest(
        ...
        Description=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["Description"].encode('utf-16-be'))]*2)]))) if "Description" in native_input.keys() else Option_None(),
        KeyUsage=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_KeyUsageType(native_input["KeyUsage"])) if "KeyUsage" in native_input.keys() else Option_None(),
        CustomerMasterKeySpec=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_CustomerMasterKeySpec(native_input["CustomerMasterKeySpec"])) if "CustomerMasterKeySpec" in native_input.keys() else Option_None(),
        KeySpec=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_KeySpec(native_input["KeySpec"])) if "KeySpec" in native_input.keys() else Option_None(),
        Origin=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_OriginType(native_input["Origin"])) if "Origin" in native_input.keys() else Option_None(),
        CustomKeyStoreId=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["CustomKeyStoreId"].encode('utf-16-be'))]*2)]))) if "CustomKeyStoreId" in native_input.keys() else Option_None(),
        ...
        Tags=Option_Some(Seq([aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_Tag(list_element) for list_element in native_input["Tags"]])) if "Tags" in native_input.keys() else Option_None(),
        MultiRegion=Option_Some(native_input["MultiRegion"]) if "MultiRegion" in native_input.keys() else Option_None(),
        XksKeyId=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["XksKeyId"].encode('utf-16-be'))]*2)]))) if "XksKeyId" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_Tag(native_input):
    return DafnyTag(
        TagKey=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["TagKey"].encode('utf-16-be'))]*2)])),
        TagValue=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["TagValue"].encode('utf-16-be'))]*2)])),
    )

def com_amazonaws_kms_CreateKeyResponse(native_input):
    return DafnyCreateKeyResponse(
        KeyMetadata=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_KeyMetadata(native_input["KeyMetadata"])) if "KeyMetadata" in native_input.keys() else Option_None(),
    )

def com_amazonaws_kms_KeyMetadata(native_input):
    return DafnyKeyMetadata(
        AWSAccountId=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["AWSAccountId"].encode('utf-16-be'))]*2)]))) if "AWSAccountId" in native_input.keys() else Option_None(),
        KeyId=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyId"].encode('utf-16-be'))]*2)])),
        Arn=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["Arn"].encode('utf-16-be'))]*2)]))) if "Arn" in native_input.keys() else Option_None(),
        CreationDate=Option_Some(_dafny.Seq(native_input["CreationDate"].isoformat())) if "CreationDate" in native_input.keys() else Option_None(),
        Enabled=Option_Some(native_input["Enabled"]) if "Enabled" in native_input.keys() else Option_None(),
        Description=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["Description"].encode('utf-16-be'))]*2)]))) if "Description" in native_input.keys() else Option_None(),
        KeyUsage=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_KeyUsageType(native_input["KeyUsage"])) if "KeyUsage" in native_input.keys() else Option_None(),
        KeyState=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_KeyState(native_input["KeyState"])) if "KeyState" in native_input.keys() else Option_None(),
        DeletionDate=Option_Some(_dafny.Seq(native_input["DeletionDate"].isoformat())) if "DeletionDate" in native_input.keys() else Option_None(),
        ValidTo=Option_Some(_dafny.Seq(native_input["ValidTo"].isoformat())) if "ValidTo" in native_input.keys() else Option_None(),
        Origin=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_OriginType(native_input["Origin"])) if "Origin" in native_input.keys() else Option_None(),
        CustomKeyStoreId=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["CustomKeyStoreId"].encode('utf-16-be'))]*2)]))) if "CustomKeyStoreId" in native_input.keys() else Option_None(),
        CloudHsmClusterId=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["CloudHsmClusterId"].encode('utf-16-be'))]*2)]))) if "CloudHsmClusterId" in native_input.keys() else Option_None(),
        ExpirationModel=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_ExpirationModelType(native_input["ExpirationModel"])) if "ExpirationModel" in native_input.keys() else Option_None(),
        KeyManager=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_KeyManagerType(native_input["KeyManager"])) if "KeyManager" in native_input.keys() else Option_None(),
        CustomerMasterKeySpec=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_CustomerMasterKeySpec(native_input["CustomerMasterKeySpec"])) if "CustomerMasterKeySpec" in native_input.keys() else Option_None(),
        KeySpec=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_KeySpec(native_input["KeySpec"])) if "KeySpec" in native_input.keys() else Option_None(),
        EncryptionAlgorithms=Option_Some(Seq([aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_EncryptionAlgorithmSpec(list_element) for list_element in native_input["EncryptionAlgorithms"]])) if "EncryptionAlgorithms" in native_input.keys() else Option_None(),
        SigningAlgorithms=Option_Some(Seq([aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_SigningAlgorithmSpec(list_element) for list_element in native_input["SigningAlgorithms"]])) if "SigningAlgorithms" in native_input.keys() else Option_None(),
        KeyAgreementAlgorithms=Option_Some(Seq([aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_KeyAgreementAlgorithmSpec(list_element) for list_element in native_input["KeyAgreementAlgorithms"]])) if "KeyAgreementAlgorithms" in native_input.keys() else Option_None(),
        MultiRegion=Option_Some(native_input["MultiRegion"]) if "MultiRegion" in native_input.keys() else Option_None(),
        MultiRegionConfiguration=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_MultiRegionConfiguration(native_input["MultiRegionConfiguration"])) if "MultiRegionConfiguration" in native_input.keys() else Option_None(),
        ...
        MacAlgorithms=Option_Some(Seq([aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_MacAlgorithmSpec(list_element) for list_element in native_input["MacAlgorithms"]])) if "MacAlgorithms" in native_input.keys() else Option_None(),
        XksKeyConfiguration=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_XksKeyConfigurationType(native_input["XksKeyConfiguration"])) if "XksKeyConfiguration" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_MultiRegionConfiguration(native_input):
    return DafnyMultiRegionConfiguration(
        MultiRegionKeyType=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_MultiRegionKeyType(native_input["MultiRegionKeyType"])) if "MultiRegionKeyType" in native_input.keys() else Option_None(),
        PrimaryKey=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_MultiRegionKey(native_input["PrimaryKey"])) if "PrimaryKey" in native_input.keys() else Option_None(),
        ReplicaKeys=Option_Some(Seq([aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_MultiRegionKey(list_element) for list_element in native_input["ReplicaKeys"]])) if "ReplicaKeys" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_XksKeyConfigurationType(native_input):
    return DafnyXksKeyConfigurationType(
        Id=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["Id"].encode('utf-16-be'))]*2)]))) if "Id" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_MultiRegionKey(native_input):
    return DafnyMultiRegionKey(
        Arn=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["Arn"].encode('utf-16-be'))]*2)]))) if "Arn" in native_input.keys() else Option_None(),
        Region=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["Region"].encode('utf-16-be'))]*2)]))) if "Region" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_DecryptRequest(native_input):
    return DafnyDecryptRequest(
        ...
        EncryptionContext=Option_Some(Map({Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(key.encode('utf-16-be'))]*2)])): Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(value.encode('utf-16-be'))]*2)])) for (key, value) in native_input["EncryptionContext"].items() })) if "EncryptionContext" in native_input.keys() else Option_None(),
        GrantTokens=Option_Some(Seq([Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(list_element.encode('utf-16-be'))]*2)])) for list_element in native_input["GrantTokens"]])) if "GrantTokens" in native_input.keys() else Option_None(),
        KeyId=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyId"].encode('utf-16-be'))]*2)]))) if "KeyId" in native_input.keys() else Option_None(),
        EncryptionAlgorithm=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_EncryptionAlgorithmSpec(native_input["EncryptionAlgorithm"])) if "EncryptionAlgorithm" in native_input.keys() else Option_None(),
        Recipient=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_RecipientInfo(native_input["Recipient"])) if "Recipient" in native_input.keys() else Option_None(),
        DryRun=Option_Some(native_input["DryRun"]) if "DryRun" in native_input.keys() else Option_None(),
    )

def com_amazonaws_kms_RecipientInfo(native_input):
    return DafnyRecipientInfo(
        KeyEncryptionAlgorithm=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_KeyEncryptionMechanism(native_input["KeyEncryptionAlgorithm"])) if "KeyEncryptionAlgorithm" in native_input.keys() else Option_None(),
        AttestationDocument=Option_Some(Seq(native_input["AttestationDocument"])) if "AttestationDocument" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_DecryptResponse(native_input):
    return DafnyDecryptResponse(
        ...
        EncryptionAlgorithm=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_EncryptionAlgorithmSpec(native_input["EncryptionAlgorithm"])) if "EncryptionAlgorithm" in native_input.keys() else Option_None(),
        CiphertextForRecipient=Option_Some(Seq(native_input["CiphertextForRecipient"])) if "CiphertextForRecipient" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_DeriveSharedSecretRequest(native_input):
    return DafnyDeriveSharedSecretRequest(
        KeyId=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyId"].encode('utf-16-be'))]*2)])),
        KeyAgreementAlgorithm=aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_KeyAgreementAlgorithmSpec(native_input["KeyAgreementAlgorithm"]),
...
def com_amazonaws_kms_DeriveSharedSecretResponse(native_input):
    return DafnyDeriveSharedSecretResponse(
        KeyId=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyId"].encode('utf-16-be'))]*2)]))) if "KeyId" in native_input.keys() else Option_None(),
        SharedSecret=Option_Some(Seq(native_input["SharedSecret"])) if "SharedSecret" in native_input.keys() else Option_None(),
        CiphertextForRecipient=Option_Some(Seq(native_input["CiphertextForRecipient"])) if "CiphertextForRecipient" in native_input.keys() else Option_None(),
        KeyAgreementAlgorithm=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_KeyAgreementAlgorithmSpec(native_input["KeyAgreementAlgorithm"])) if "KeyAgreementAlgorithm" in native_input.keys() else Option_None(),
        KeyOrigin=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_OriginType(native_input["KeyOrigin"])) if "KeyOrigin" in native_input.keys() else Option_None(),
    )

def com_amazonaws_kms_DescribeCustomKeyStoresRequest(native_input):
    return DafnyDescribeCustomKeyStoresRequest(
        CustomKeyStoreId=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["CustomKeyStoreId"].encode('utf-16-be'))]*2)]))) if "CustomKeyStoreId" in native_input.keys() else Option_None(),
        CustomKeyStoreName=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["CustomKeyStoreName"].encode('utf-16-be'))]*2)]))) if "CustomKeyStoreName" in native_input.keys() else Option_None(),
        Limit=Option_Some(native_input["Limit"]) if "Limit" in native_input.keys() else Option_None(),
        Marker=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["Marker"].encode('utf-16-be'))]*2)]))) if "Marker" in native_input.keys() else Option_None(),
    )

def com_amazonaws_kms_DescribeCustomKeyStoresResponse(native_input):
    return DafnyDescribeCustomKeyStoresResponse(
        CustomKeyStores=Option_Some(Seq([aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_CustomKeyStoresListEntry(list_element) for list_element in native_input["CustomKeyStores"]])) if "CustomKeyStores" in native_input.keys() else Option_None(),
        NextMarker=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["NextMarker"].encode('utf-16-be'))]*2)]))) if "NextMarker" in native_input.keys() else Option_None(),
...
def com_amazonaws_kms_CustomKeyStoresListEntry(native_input):
    return DafnyCustomKeyStoresListEntry(
        ...
        CloudHsmClusterId=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["CloudHsmClusterId"].encode('utf-16-be'))]*2)]))) if "CloudHsmClusterId" in native_input.keys() else Option_None(),
        TrustAnchorCertificate=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["TrustAnchorCertificate"].encode('utf-16-be'))]*2)]))) if "TrustAnchorCertificate" in native_input.keys() else Option_None(),
        ConnectionState=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_ConnectionStateType(native_input["ConnectionState"])) if "ConnectionState" in native_input.keys() else Option_None(),
        ConnectionErrorCode=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_ConnectionErrorCodeType(native_input["ConnectionErrorCode"])) if "ConnectionErrorCode" in native_input.keys() else Option_None(),
        CreationDate=Option_Some(_dafny.Seq(native_input["CreationDate"].isoformat())) if "CreationDate" in native_input.keys() else Option_None(),
        CustomKeyStoreType=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_CustomKeyStoreType(native_input["CustomKeyStoreType"])) if "CustomKeyStoreType" in native_input.keys() else Option_None(),
        XksProxyConfiguration=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_XksProxyConfigurationType(native_input["XksProxyConfiguration"])) if "XksProxyConfiguration" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_ConnectionErrorCodeType(native_input):
    ...
    if native_input == "INVALID_CREDENTIALS":
        ...
        return ConnectionErrorCodeType_XKS__VPC__ENDPOINT__SERVICE__INVALID__CONFIGURATION()
...
def com_amazonaws_kms_XksProxyConfigurationType(native_input):
    return DafnyXksProxyConfigurationType(
        Connectivity=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_XksProxyConnectivityType(native_input["Connectivity"])) if "Connectivity" in native_input.keys() else Option_None(),
        AccessKeyId=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["AccessKeyId"].encode('utf-16-be'))]*2)]))) if "AccessKeyId" in native_input.keys() else Option_None(),
        UriEndpoint=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["UriEndpoint"].encode('utf-16-be'))]*2)]))) if "UriEndpoint" in native_input.keys() else Option_None(),
        UriPath=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["UriPath"].encode('utf-16-be'))]*2)]))) if "UriPath" in native_input.keys() else Option_None(),
        VpcEndpointServiceName=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["VpcEndpointServiceName"].encode('utf-16-be'))]*2)]))) if "VpcEndpointServiceName" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_EnableKeyRotationRequest(native_input):
    return DafnyEnableKeyRotationRequest(
        KeyId=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyId"].encode('utf-16-be'))]*2)])),
        RotationPeriodInDays=Option_Some(native_input["RotationPeriodInDays"]) if "RotationPeriodInDays" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_GenerateDataKeyRequest(native_input):
    return DafnyGenerateDataKeyRequest(
        ...
        KeySpec=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_DataKeySpec(native_input["KeySpec"])) if "KeySpec" in native_input.keys() else Option_None(),
        GrantTokens=Option_Some(Seq([Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(list_element.encode('utf-16-be'))]*2)])) for list_element in native_input["GrantTokens"]])) if "GrantTokens" in native_input.keys() else Option_None(),
        Recipient=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_RecipientInfo(native_input["Recipient"])) if "Recipient" in native_input.keys() else Option_None(),
        DryRun=Option_Some(native_input["DryRun"]) if "DryRun" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_GenerateDataKeyPairRequest(native_input):
    return DafnyGenerateDataKeyPairRequest(
        EncryptionContext=Option_Some(Map({Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(key.encode('utf-16-be'))]*2)])): Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(value.encode('utf-16-be'))]*2)])) for (key, value) in native_input["EncryptionContext"].items() })) if "EncryptionContext" in native_input.keys() else Option_None(),
        KeyId=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyId"].encode('utf-16-be'))]*2)])),
        KeyPairSpec=aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_DataKeyPairSpec(native_input["KeyPairSpec"]),
        GrantTokens=Option_Some(Seq([Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(list_element.encode('utf-16-be'))]*2)])) for list_element in native_input["GrantTokens"]])) if "GrantTokens" in native_input.keys() else Option_None(),
        Recipient=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_RecipientInfo(native_input["Recipient"])) if "Recipient" in native_input.keys() else Option_None(),
        DryRun=Option_Some(native_input["DryRun"]) if "DryRun" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_GenerateDataKeyPairResponse(native_input):
    return DafnyGenerateDataKeyPairResponse(
        ...
        KeyId=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyId"].encode('utf-16-be'))]*2)]))) if "KeyId" in native_input.keys() else Option_None(),
        KeyPairSpec=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_DataKeyPairSpec(native_input["KeyPairSpec"])) if "KeyPairSpec" in native_input.keys() else Option_None(),
        CiphertextForRecipient=Option_Some(Seq(native_input["CiphertextForRecipient"])) if "CiphertextForRecipient" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_GenerateMacRequest(native_input):
    return DafnyGenerateMacRequest(
        ...
        KeyId=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyId"].encode('utf-16-be'))]*2)])),
        MacAlgorithm=aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_MacAlgorithmSpec(native_input["MacAlgorithm"]),
        GrantTokens=Option_Some(Seq([Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(list_element.encode('utf-16-be'))]*2)])) for list_element in native_input["GrantTokens"]])) if "GrantTokens" in native_input.keys() else Option_None(),
        DryRun=Option_Some(native_input["DryRun"]) if "DryRun" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_GenerateMacResponse(native_input):
    return DafnyGenerateMacResponse(
        Mac=Option_Some(Seq(native_input["Mac"])) if "Mac" in native_input.keys() else Option_None(),
        MacAlgorithm=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_MacAlgorithmSpec(native_input["MacAlgorithm"])) if "MacAlgorithm" in native_input.keys() else Option_None(),
        KeyId=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyId"].encode('utf-16-be'))]*2)]))) if "KeyId" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_GetKeyRotationStatusResponse(native_input):
    return DafnyGetKeyRotationStatusResponse(
        ...
        KeyId=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyId"].encode('utf-16-be'))]*2)]))) if "KeyId" in native_input.keys() else Option_None(),
        RotationPeriodInDays=Option_Some(native_input["RotationPeriodInDays"]) if "RotationPeriodInDays" in native_input.keys() else Option_None(),
        NextRotationDate=Option_Some(_dafny.Seq(native_input["NextRotationDate"].isoformat())) if "NextRotationDate" in native_input.keys() else Option_None(),
        OnDemandRotationStartDate=Option_Some(_dafny.Seq(native_input["OnDemandRotationStartDate"].isoformat())) if "OnDemandRotationStartDate" in native_input.keys() else Option_None(),
    )

def com_amazonaws_kms_GetParametersForImportRequest(native_input):
    return DafnyGetParametersForImportRequest(
        KeyId=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyId"].encode('utf-16-be'))]*2)])),
        WrappingAlgorithm=aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_AlgorithmSpec(native_input["WrappingAlgorithm"]),
        WrappingKeySpec=aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_WrappingKeySpec(native_input["WrappingKeySpec"]),
    )
...
def com_amazonaws_kms_GetParametersForImportResponse(native_input):
    return DafnyGetParametersForImportResponse(
        ...
        PublicKey=Option_Some(Seq(native_input["PublicKey"])) if "PublicKey" in native_input.keys() else Option_None(),
        ParametersValidTo=Option_Some(_dafny.Seq(native_input["ParametersValidTo"].isoformat())) if "ParametersValidTo" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_ListAliasesResponse(native_input):
    return DafnyListAliasesResponse(
        Aliases=Option_Some(Seq([aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_AliasListEntry(list_element) for list_element in native_input["Aliases"]])) if "Aliases" in native_input.keys() else Option_None(),
        NextMarker=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["NextMarker"].encode('utf-16-be'))]*2)]))) if "NextMarker" in native_input.keys() else Option_None(),
        Truncated=Option_Some(native_input["Truncated"]) if "Truncated" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_AliasListEntry(native_input):
    return DafnyAliasListEntry(
        AliasName=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["AliasName"].encode('utf-16-be'))]*2)]))) if "AliasName" in native_input.keys() else Option_None(),
        AliasArn=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["AliasArn"].encode('utf-16-be'))]*2)]))) if "AliasArn" in native_input.keys() else Option_None(),
        TargetKeyId=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["TargetKeyId"].encode('utf-16-be'))]*2)]))) if "TargetKeyId" in native_input.keys() else Option_None(),
        CreationDate=Option_Some(_dafny.Seq(native_input["CreationDate"].isoformat())) if "CreationDate" in native_input.keys() else Option_None(),
        LastUpdatedDate=Option_Some(_dafny.Seq(native_input["LastUpdatedDate"].isoformat())) if "LastUpdatedDate" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_ListGrantsRequest(native_input):
    return DafnyListGrantsRequest(
        ...
        KeyId=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyId"].encode('utf-16-be'))]*2)])),
        GrantId=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["GrantId"].encode('utf-16-be'))]*2)]))) if "GrantId" in native_input.keys() else Option_None(),
        GranteePrincipal=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["GranteePrincipal"].encode('utf-16-be'))]*2)]))) if "GranteePrincipal" in native_input.keys() else Option_None(),
    )

def com_amazonaws_kms_ListGrantsResponse(native_input):
    return DafnyListGrantsResponse(
        Grants=Option_Some(Seq([aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_GrantListEntry(list_element) for list_element in native_input["Grants"]])) if "Grants" in native_input.keys() else Option_None(),
        NextMarker=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["NextMarker"].encode('utf-16-be'))]*2)]))) if "NextMarker" in native_input.keys() else Option_None(),
        Truncated=Option_Some(native_input["Truncated"]) if "Truncated" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_GrantListEntry(native_input):
    return DafnyGrantListEntry(
        ...
        GranteePrincipal=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["GranteePrincipal"].encode('utf-16-be'))]*2)]))) if "GranteePrincipal" in native_input.keys() else Option_None(),
        RetiringPrincipal=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["RetiringPrincipal"].encode('utf-16-be'))]*2)]))) if "RetiringPrincipal" in native_input.keys() else Option_None(),
        IssuingAccount=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["IssuingAccount"].encode('utf-16-be'))]*2)]))) if "IssuingAccount" in native_input.keys() else Option_None(),
        Operations=Option_Some(Seq([aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_GrantOperation(list_element) for list_element in native_input["Operations"]])) if "Operations" in native_input.keys() else Option_None(),
        Constraints=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_GrantConstraints(native_input["Constraints"])) if "Constraints" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_ListKeyPoliciesResponse(native_input):
    return DafnyListKeyPoliciesResponse(
        PolicyNames=Option_Some(Seq([Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(list_element.encode('utf-16-be'))]*2)])) for list_element in native_input["PolicyNames"]])) if "PolicyNames" in native_input.keys() else Option_None(),
        NextMarker=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["NextMarker"].encode('utf-16-be'))]*2)]))) if "NextMarker" in native_input.keys() else Option_None(),
        Truncated=Option_Some(native_input["Truncated"]) if "Truncated" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_ListKeyRotationsResponse(native_input):
    return DafnyListKeyRotationsResponse(
        Rotations=Option_Some(Seq([aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_RotationsListEntry(list_element) for list_element in native_input["Rotations"]])) if "Rotations" in native_input.keys() else Option_None(),
        NextMarker=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["NextMarker"].encode('utf-16-be'))]*2)]))) if "NextMarker" in native_input.keys() else Option_None(),
        Truncated=Option_Some(native_input["Truncated"]) if "Truncated" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_RotationsListEntry(native_input):
    return DafnyRotationsListEntry(
        KeyId=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyId"].encode('utf-16-be'))]*2)]))) if "KeyId" in native_input.keys() else Option_None(),
        RotationDate=Option_Some(_dafny.Seq(native_input["RotationDate"].isoformat())) if "RotationDate" in native_input.keys() else Option_None(),
        RotationType=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_RotationType(native_input["RotationType"])) if "RotationType" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_ListKeysResponse(native_input):
    return DafnyListKeysResponse(
        Keys=Option_Some(Seq([aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_KeyListEntry(list_element) for list_element in native_input["Keys"]])) if "Keys" in native_input.keys() else Option_None(),
        NextMarker=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["NextMarker"].encode('utf-16-be'))]*2)]))) if "NextMarker" in native_input.keys() else Option_None(),
        Truncated=Option_Some(native_input["Truncated"]) if "Truncated" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_KeyListEntry(native_input):
    return DafnyKeyListEntry(
        KeyId=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyId"].encode('utf-16-be'))]*2)]))) if "KeyId" in native_input.keys() else Option_None(),
        KeyArn=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyArn"].encode('utf-16-be'))]*2)]))) if "KeyArn" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_ReEncryptRequest(native_input):
    return DafnyReEncryptRequest(
        CiphertextBlob=Seq(native_input["CiphertextBlob"]),
        SourceEncryptionContext=Option_Some(Map({Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(key.encode('utf-16-be'))]*2)])): Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(value.encode('utf-16-be'))]*2)])) for (key, value) in native_input["SourceEncryptionContext"].items() })) if "SourceEncryptionContext" in native_input.keys() else Option_None(),
        SourceKeyId=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["SourceKeyId"].encode('utf-16-be'))]*2)]))) if "SourceKeyId" in native_input.keys() else Option_None(),
        DestinationKeyId=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["DestinationKeyId"].encode('utf-16-be'))]*2)])),
        DestinationEncryptionContext=Option_Some(Map({Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(key.encode('utf-16-be'))]*2)])): Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(value.encode('utf-16-be'))]*2)])) for (key, value) in native_input["DestinationEncryptionContext"].items() })) if "DestinationEncryptionContext" in native_input.keys() else Option_None(),
        SourceEncryptionAlgorithm=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_EncryptionAlgorithmSpec(native_input["SourceEncryptionAlgorithm"])) if "SourceEncryptionAlgorithm" in native_input.keys() else Option_None(),
        DestinationEncryptionAlgorithm=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_EncryptionAlgorithmSpec(native_input["DestinationEncryptionAlgorithm"])) if "DestinationEncryptionAlgorithm" in native_input.keys() else Option_None(),
        GrantTokens=Option_Some(Seq([Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(list_element.encode('utf-16-be'))]*2)])) for list_element in native_input["GrantTokens"]])) if "GrantTokens" in native_input.keys() else Option_None(),
        DryRun=Option_Some(native_input["DryRun"]) if "DryRun" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_ReplicateKeyRequest(native_input):
    return DafnyReplicateKeyRequest(
        KeyId=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyId"].encode('utf-16-be'))]*2)])),
        ReplicaRegion=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["ReplicaRegion"].encode('utf-16-be'))]*2)])),
        Policy=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["Policy"].encode('utf-16-be'))]*2)]))) if "Policy" in native_input.keys() else Option_None(),
        BypassPolicyLockoutSafetyCheck=Option_Some(native_input["BypassPolicyLockoutSafetyCheck"]) if "BypassPolicyLockoutSafetyCheck" in native_input.keys() else Option_None(),
        Description=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["Description"].encode('utf-16-be'))]*2)]))) if "Description" in native_input.keys() else Option_None(),
        Tags=Option_Some(Seq([aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_Tag(list_element) for list_element in native_input["Tags"]])) if "Tags" in native_input.keys() else Option_None(),
...
def com_amazonaws_kms_ReplicateKeyResponse(native_input):
    return DafnyReplicateKeyResponse(
        ReplicaKeyMetadata=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_KeyMetadata(native_input["ReplicaKeyMetadata"])) if "ReplicaKeyMetadata" in native_input.keys() else Option_None(),
        ReplicaPolicy=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["ReplicaPolicy"].encode('utf-16-be'))]*2)]))) if "ReplicaPolicy" in native_input.keys() else Option_None(),
        ReplicaTags=Option_Some(Seq([aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_Tag(list_element) for list_element in native_input["ReplicaTags"]])) if "ReplicaTags" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_RevokeGrantRequest(native_input):
    return DafnyRevokeGrantRequest(
        KeyId=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyId"].encode('utf-16-be'))]*2)])),
        GrantId=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["GrantId"].encode('utf-16-be'))]*2)])),
        DryRun=Option_Some(native_input["DryRun"]) if "DryRun" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_SignRequest(native_input):
    return DafnySignRequest(
        KeyId=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyId"].encode('utf-16-be'))]*2)])),
        Message=Seq(native_input["Message"]),
        MessageType=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_MessageType(native_input["MessageType"])) if "MessageType" in native_input.keys() else Option_None(),
        GrantTokens=Option_Some(Seq([Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(list_element.encode('utf-16-be'))]*2)])) for list_element in native_input["GrantTokens"]])) if "GrantTokens" in native_input.keys() else Option_None(),
        SigningAlgorithm=aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_SigningAlgorithmSpec(native_input["SigningAlgorithm"]),
        DryRun=Option_Some(native_input["DryRun"]) if "DryRun" in native_input.keys() else Option_None(),
    )
...
def com_amazonaws_kms_SignResponse(native_input):
    return DafnySignResponse(
        ...
        SigningAlgorithm=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_SigningAlgorithmSpec(native_input["SigningAlgorithm"])) if "SigningAlgorithm" in native_input.keys() else Option_None(),
    )

def com_amazonaws_kms_TagResourceRequest(native_input):
    return DafnyTagResourceRequest(
        KeyId=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyId"].encode('utf-16-be'))]*2)])),
        Tags=Seq([aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_Tag(list_element) for list_element in native_input["Tags"]]),
    )
...
def com_amazonaws_kms_UpdateCustomKeyStoreRequest(native_input):
    return DafnyUpdateCustomKeyStoreRequest(
        CustomKeyStoreId=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["CustomKeyStoreId"].encode('utf-16-be'))]*2)])),
        NewCustomKeyStoreName=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["NewCustomKeyStoreName"].encode('utf-16-be'))]*2)]))) if "NewCustomKeyStoreName" in native_input.keys() else Option_None(),
        KeyStorePassword=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyStorePassword"].encode('utf-16-be'))]*2)]))) if "KeyStorePassword" in native_input.keys() else Option_None(),
        CloudHsmClusterId=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["CloudHsmClusterId"].encode('utf-16-be'))]*2)]))) if "CloudHsmClusterId" in native_input.keys() else Option_None(),
        XksProxyUriEndpoint=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["XksProxyUriEndpoint"].encode('utf-16-be'))]*2)]))) if "XksProxyUriEndpoint" in native_input.keys() else Option_None(),
        XksProxyUriPath=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["XksProxyUriPath"].encode('utf-16-be'))]*2)]))) if "XksProxyUriPath" in native_input.keys() else Option_None(),
...
def com_amazonaws_kms_UpdateKeyDescriptionRequest(native_input):
    return DafnyUpdateKeyDescriptionRequest(
        KeyId=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyId"].encode('utf-16-be'))]*2)])),
        Description=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["Description"].encode('utf-16-be'))]*2)])),
    )

def com_amazonaws_kms_UpdatePrimaryRegionRequest(native_input):
    return DafnyUpdatePrimaryRegionRequest(
        KeyId=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyId"].encode('utf-16-be'))]*2)])),
        PrimaryRegion=Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["PrimaryRegion"].encode('utf-16-be'))]*2)])),
    )
...
def com_amazonaws_kms_VerifyMacResponse(native_input):
    return DafnyVerifyMacResponse(
        KeyId=Option_Some(Seq(''.join([chr(int.from_bytes(pair, 'big')) for pair in zip(*[iter(native_input["KeyId"].encode('utf-16-be'))]*2)]))) if "KeyId" in native_input.keys() else Option_None(),
        MacValid=Option_Some(native_input["MacValid"]) if "MacValid" in native_input.keys() else Option_None(),
        MacAlgorithm=Option_Some(aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.aws_sdk_to_dafny.com_amazonaws_kms_MacAlgorithmSpec(native_input["MacAlgorithm"])) if "MacAlgorithm" in native_input.keys() else Option_None(),
    )
