...
def com_amazonaws_kms_CancelKeyDeletionRequest(dafny_input):
    output = {}
    output["KeyId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.KeyId).decode('utf-16-be')
    return output

def com_amazonaws_kms_CancelKeyDeletionResponse(dafny_input):
    output = {}
    if dafny_input.KeyId.is_Some:
        output["KeyId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.KeyId.value).decode('utf-16-be')

    return output

def com_amazonaws_kms_ConnectCustomKeyStoreRequest(dafny_input):
    output = {}
    output["CustomKeyStoreId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.CustomKeyStoreId).decode('utf-16-be')
    return output
...
def com_amazonaws_kms_CreateAliasRequest(dafny_input):
    output = {}
    output["AliasName"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.AliasName).decode('utf-16-be')
    output["TargetKeyId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.TargetKeyId).decode('utf-16-be')
    return output
...
def com_amazonaws_kms_XksProxyAuthenticationCredentialType(dafny_input):
    output = {}
    output["AccessKeyId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.AccessKeyId).decode('utf-16-be')
    output["RawSecretAccessKey"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.RawSecretAccessKey).decode('utf-16-be')
    return output
...
def com_amazonaws_kms_CreateCustomKeyStoreRequest(dafny_input):
    output = {}
    output["CustomKeyStoreName"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.CustomKeyStoreName).decode('utf-16-be')
    if dafny_input.CloudHsmClusterId.is_Some:
        output["CloudHsmClusterId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.CloudHsmClusterId.value).decode('utf-16-be')

    if dafny_input.TrustAnchorCertificate.is_Some:
        output["TrustAnchorCertificate"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.TrustAnchorCertificate.value).decode('utf-16-be')

    if dafny_input.KeyStorePassword.is_Some:
        output["KeyStorePassword"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.KeyStorePassword.value).decode('utf-16-be')

    if dafny_input.CustomKeyStoreType.is_Some:
        output["CustomKeyStoreType"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_CustomKeyStoreType(dafny_input.CustomKeyStoreType.value)

    if dafny_input.XksProxyUriEndpoint.is_Some:
        output["XksProxyUriEndpoint"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.XksProxyUriEndpoint.value).decode('utf-16-be')

    if dafny_input.XksProxyUriPath.is_Some:
        output["XksProxyUriPath"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.XksProxyUriPath.value).decode('utf-16-be')

    if dafny_input.XksProxyVpcEndpointServiceName.is_Some:
        output["XksProxyVpcEndpointServiceName"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.XksProxyVpcEndpointServiceName.value).decode('utf-16-be')

    if dafny_input.XksProxyAuthenticationCredential.is_Some:
        output["XksProxyAuthenticationCredential"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_XksProxyAuthenticationCredentialType(dafny_input.XksProxyAuthenticationCredential.value)

    if dafny_input.XksProxyConnectivity.is_Some:
        output["XksProxyConnectivity"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_XksProxyConnectivityType(dafny_input.XksProxyConnectivity.value)
...
def com_amazonaws_kms_CreateCustomKeyStoreResponse(dafny_input):
    ...
    if dafny_input.CustomKeyStoreId.is_Some:
        output["CustomKeyStoreId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.CustomKeyStoreId.value).decode('utf-16-be')
...
def com_amazonaws_kms_GrantConstraints(dafny_input):
    ...
    if dafny_input.EncryptionContextSubset.is_Some:
        output["EncryptionContextSubset"] = {b''.join(ord(c).to_bytes(2, 'big') for c in key).decode('utf-16-be'): b''.join(ord(c).to_bytes(2, 'big') for c in value).decode('utf-16-be') for (key, value) in dafny_input.EncryptionContextSubset.value.items }

    if dafny_input.EncryptionContextEquals.is_Some:
        output["EncryptionContextEquals"] = {b''.join(ord(c).to_bytes(2, 'big') for c in key).decode('utf-16-be'): b''.join(ord(c).to_bytes(2, 'big') for c in value).decode('utf-16-be') for (key, value) in dafny_input.EncryptionContextEquals.value.items }
...
def com_amazonaws_kms_CreateGrantRequest(dafny_input):
    output = {}
    output["KeyId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.KeyId).decode('utf-16-be')
    output["GranteePrincipal"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.GranteePrincipal).decode('utf-16-be')
    if dafny_input.RetiringPrincipal.is_Some:
        output["RetiringPrincipal"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.RetiringPrincipal.value).decode('utf-16-be')

    output["Operations"] = [aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_GrantOperation(list_element) for list_element in dafny_input.Operations]
    if dafny_input.Constraints.is_Some:
        output["Constraints"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_GrantConstraints(dafny_input.Constraints.value)

    if dafny_input.GrantTokens.is_Some:
        output["GrantTokens"] = [b''.join(ord(c).to_bytes(2, 'big') for c in list_element).decode('utf-16-be') for list_element in dafny_input.GrantTokens.value]

    if dafny_input.Name.is_Some:
        output["Name"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.Name.value).decode('utf-16-be')
...
def com_amazonaws_kms_CreateGrantResponse(dafny_input):
    ...
    if dafny_input.GrantToken.is_Some:
        output["GrantToken"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.GrantToken.value).decode('utf-16-be')

    if dafny_input.GrantId.is_Some:
        output["GrantId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.GrantId.value).decode('utf-16-be')
...
def com_amazonaws_kms_Tag(dafny_input):
    output = {}
    output["TagKey"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.TagKey).decode('utf-16-be')
    output["TagValue"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.TagValue).decode('utf-16-be')
    return output
...
def com_amazonaws_kms_CreateKeyRequest(dafny_input):
    ...
    if dafny_input.Description.is_Some:
        output["Description"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.Description.value).decode('utf-16-be')

    if dafny_input.KeyUsage.is_Some:
        output["KeyUsage"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_KeyUsageType(dafny_input.KeyUsage.value)

    if dafny_input.CustomerMasterKeySpec.is_Some:
        output["CustomerMasterKeySpec"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_CustomerMasterKeySpec(dafny_input.CustomerMasterKeySpec.value)

    if dafny_input.KeySpec.is_Some:
        output["KeySpec"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_KeySpec(dafny_input.KeySpec.value)

    if dafny_input.Origin.is_Some:
        output["Origin"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_OriginType(dafny_input.Origin.value)
    ...
    if dafny_input.Tags.is_Some:
        output["Tags"] = [aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_Tag(list_element) for list_element in dafny_input.Tags.value]

    if dafny_input.MultiRegion.is_Some:
        output["MultiRegion"] = dafny_input.MultiRegion.value

    if dafny_input.XksKeyId.is_Some:
        output["XksKeyId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.XksKeyId.value).decode('utf-16-be')
...
def com_amazonaws_kms_KeyMetadata(dafny_input):
    ...
    if dafny_input.AWSAccountId.is_Some:
        output["AWSAccountId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.AWSAccountId.value).decode('utf-16-be')

    output["KeyId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.KeyId).decode('utf-16-be')
    if dafny_input.Arn.is_Some:
        output["Arn"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.Arn.value).decode('utf-16-be')

    if dafny_input.CreationDate.is_Some:
        output["CreationDate"] = datetime.fromisoformat(dafny_input.CreationDate.value.VerbatimString(False))
    ...
    if dafny_input.KeyState.is_Some:
        output["KeyState"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_KeyState(dafny_input.KeyState.value)

    if dafny_input.DeletionDate.is_Some:
        output["DeletionDate"] = datetime.fromisoformat(dafny_input.DeletionDate.value.VerbatimString(False))

    if dafny_input.ValidTo.is_Some:
        output["ValidTo"] = datetime.fromisoformat(dafny_input.ValidTo.value.VerbatimString(False))
    ...
    if dafny_input.ExpirationModel.is_Some:
        output["ExpirationModel"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_ExpirationModelType(dafny_input.ExpirationModel.value)

    if dafny_input.KeyManager.is_Some:
        output["KeyManager"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_KeyManagerType(dafny_input.KeyManager.value)
    ...
    if dafny_input.EncryptionAlgorithms.is_Some:
        output["EncryptionAlgorithms"] = [aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_EncryptionAlgorithmSpec(list_element) for list_element in dafny_input.EncryptionAlgorithms.value]

    if dafny_input.SigningAlgorithms.is_Some:
        output["SigningAlgorithms"] = [aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_SigningAlgorithmSpec(list_element) for list_element in dafny_input.SigningAlgorithms.value]

    if dafny_input.KeyAgreementAlgorithms.is_Some:
        output["KeyAgreementAlgorithms"] = [aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_KeyAgreementAlgorithmSpec(list_element) for list_element in dafny_input.KeyAgreementAlgorithms.value]
    ...
    if dafny_input.MacAlgorithms.is_Some:
        output["MacAlgorithms"] = [aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_MacAlgorithmSpec(list_element) for list_element in dafny_input.MacAlgorithms.value]

    if dafny_input.XksKeyConfiguration.is_Some:
        output["XksKeyConfiguration"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_XksKeyConfigurationType(dafny_input.XksKeyConfiguration.value)
...
def com_amazonaws_kms_MultiRegionConfiguration(dafny_input):
    ...
    if dafny_input.MultiRegionKeyType.is_Some:
        output["MultiRegionKeyType"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_MultiRegionKeyType(dafny_input.MultiRegionKeyType.value)

    if dafny_input.PrimaryKey.is_Some:
        output["PrimaryKey"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_MultiRegionKey(dafny_input.PrimaryKey.value)

    if dafny_input.ReplicaKeys.is_Some:
        output["ReplicaKeys"] = [aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_MultiRegionKey(list_element) for list_element in dafny_input.ReplicaKeys.value]
...
def com_amazonaws_kms_XksKeyConfigurationType(dafny_input):
    ...
    if dafny_input.Id.is_Some:
        output["Id"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.Id.value).decode('utf-16-be')
...
def com_amazonaws_kms_MultiRegionKey(dafny_input):
    ...
    if dafny_input.Region.is_Some:
        output["Region"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.Region.value).decode('utf-16-be')
...
def com_amazonaws_kms_CreateKeyResponse(dafny_input):
    ...
    if dafny_input.KeyMetadata.is_Some:
        output["KeyMetadata"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_KeyMetadata(dafny_input.KeyMetadata.value)
...
def com_amazonaws_kms_RecipientInfo(dafny_input):
    ...
    if dafny_input.KeyEncryptionAlgorithm.is_Some:
        output["KeyEncryptionAlgorithm"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_KeyEncryptionMechanism(dafny_input.KeyEncryptionAlgorithm.value)
...
def com_amazonaws_kms_DecryptRequest(dafny_input):
    ...
    if dafny_input.EncryptionContext.is_Some:
        output["EncryptionContext"] = {b''.join(ord(c).to_bytes(2, 'big') for c in key).decode('utf-16-be'): b''.join(ord(c).to_bytes(2, 'big') for c in value).decode('utf-16-be') for (key, value) in dafny_input.EncryptionContext.value.items }
    ...
    if dafny_input.EncryptionAlgorithm.is_Some:
        output["EncryptionAlgorithm"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_EncryptionAlgorithmSpec(dafny_input.EncryptionAlgorithm.value)

    if dafny_input.Recipient.is_Some:
        output["Recipient"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_RecipientInfo(dafny_input.Recipient.value)
...
def com_amazonaws_kms_DeriveSharedSecretRequest(dafny_input):
    output = {}
    output["KeyId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.KeyId).decode('utf-16-be')
    output["KeyAgreementAlgorithm"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_KeyAgreementAlgorithmSpec(dafny_input.KeyAgreementAlgorithm)
...
def com_amazonaws_kms_DeriveSharedSecretResponse(dafny_input):
    ...
    if dafny_input.KeyAgreementAlgorithm.is_Some:
        output["KeyAgreementAlgorithm"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_KeyAgreementAlgorithmSpec(dafny_input.KeyAgreementAlgorithm.value)

    if dafny_input.KeyOrigin.is_Some:
        output["KeyOrigin"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_OriginType(dafny_input.KeyOrigin.value)
...
def com_amazonaws_kms_DescribeCustomKeyStoresRequest(dafny_input):
    ...
    if dafny_input.CustomKeyStoreName.is_Some:
        output["CustomKeyStoreName"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.CustomKeyStoreName.value).decode('utf-16-be')

    if dafny_input.Limit.is_Some:
        output["Limit"] = dafny_input.Limit.value

    if dafny_input.Marker.is_Some:
        output["Marker"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.Marker.value).decode('utf-16-be')
...
def com_amazonaws_kms_CustomKeyStoresListEntry(dafny_input):
    ...
    if dafny_input.ConnectionState.is_Some:
        output["ConnectionState"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_ConnectionStateType(dafny_input.ConnectionState.value)

    if dafny_input.ConnectionErrorCode.is_Some:
        output["ConnectionErrorCode"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_ConnectionErrorCodeType(dafny_input.ConnectionErrorCode.value)
    ...
    if dafny_input.XksProxyConfiguration.is_Some:
        output["XksProxyConfiguration"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_XksProxyConfigurationType(dafny_input.XksProxyConfiguration.value)
...
def com_amazonaws_kms_XksProxyConfigurationType(dafny_input):
    ...
    if dafny_input.Connectivity.is_Some:
        output["Connectivity"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_XksProxyConnectivityType(dafny_input.Connectivity.value)

    if dafny_input.AccessKeyId.is_Some:
        output["AccessKeyId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.AccessKeyId.value).decode('utf-16-be')

    if dafny_input.UriEndpoint.is_Some:
        output["UriEndpoint"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.UriEndpoint.value).decode('utf-16-be')

    if dafny_input.UriPath.is_Some:
        output["UriPath"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.UriPath.value).decode('utf-16-be')

    if dafny_input.VpcEndpointServiceName.is_Some:
        output["VpcEndpointServiceName"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.VpcEndpointServiceName.value).decode('utf-16-be')
...
def com_amazonaws_kms_DescribeCustomKeyStoresResponse(dafny_input):
    ...
    if dafny_input.CustomKeyStores.is_Some:
        output["CustomKeyStores"] = [aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_CustomKeyStoresListEntry(list_element) for list_element in dafny_input.CustomKeyStores.value]

    if dafny_input.NextMarker.is_Some:
        output["NextMarker"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.NextMarker.value).decode('utf-16-be')
...
def com_amazonaws_kms_GenerateDataKeyRequest(dafny_input):
    ...
    if dafny_input.KeySpec.is_Some:
        output["KeySpec"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_DataKeySpec(dafny_input.KeySpec.value)
...
def com_amazonaws_kms_GenerateDataKeyPairRequest(dafny_input):
    ...
    output["KeyId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.KeyId).decode('utf-16-be')
    output["KeyPairSpec"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_DataKeyPairSpec(dafny_input.KeyPairSpec)
    if dafny_input.GrantTokens.is_Some:
        output["GrantTokens"] = [b''.join(ord(c).to_bytes(2, 'big') for c in list_element).decode('utf-16-be') for list_element in dafny_input.GrantTokens.value]
...
def com_amazonaws_kms_GenerateDataKeyPairResponse(dafny_input):
    ...
    if dafny_input.KeyPairSpec.is_Some:
        output["KeyPairSpec"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_DataKeyPairSpec(dafny_input.KeyPairSpec.value)
...
def com_amazonaws_kms_GenerateMacRequest(dafny_input):
    ...
    output["KeyId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.KeyId).decode('utf-16-be')
    output["MacAlgorithm"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_MacAlgorithmSpec(dafny_input.MacAlgorithm)
    if dafny_input.GrantTokens.is_Some:
        output["GrantTokens"] = [b''.join(ord(c).to_bytes(2, 'big') for c in list_element).decode('utf-16-be') for list_element in dafny_input.GrantTokens.value]
...
def com_amazonaws_kms_GenerateMacResponse(dafny_input):
    ...
    if dafny_input.MacAlgorithm.is_Some:
        output["MacAlgorithm"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_MacAlgorithmSpec(dafny_input.MacAlgorithm.value)
...
def com_amazonaws_kms_GetKeyRotationStatusResponse(dafny_input):
    ...
    if dafny_input.NextRotationDate.is_Some:
        output["NextRotationDate"] = datetime.fromisoformat(dafny_input.NextRotationDate.value.VerbatimString(False))

    if dafny_input.OnDemandRotationStartDate.is_Some:
        output["OnDemandRotationStartDate"] = datetime.fromisoformat(dafny_input.OnDemandRotationStartDate.value.VerbatimString(False))
...
def com_amazonaws_kms_GetParametersForImportRequest(dafny_input):
    output = {}
    output["KeyId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.KeyId).decode('utf-16-be')
    output["WrappingAlgorithm"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_AlgorithmSpec(dafny_input.WrappingAlgorithm)
    output["WrappingKeySpec"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_WrappingKeySpec(dafny_input.WrappingKeySpec)
    return output
...
def com_amazonaws_kms_GetParametersForImportResponse(dafny_input):
    ...
    if dafny_input.ParametersValidTo.is_Some:
        output["ParametersValidTo"] = datetime.fromisoformat(dafny_input.ParametersValidTo.value.VerbatimString(False))
...
def com_amazonaws_kms_AliasListEntry(dafny_input):
    ...
    if dafny_input.AliasName.is_Some:
        output["AliasName"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.AliasName.value).decode('utf-16-be')

    if dafny_input.AliasArn.is_Some:
        output["AliasArn"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.AliasArn.value).decode('utf-16-be')

    if dafny_input.TargetKeyId.is_Some:
        output["TargetKeyId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.TargetKeyId.value).decode('utf-16-be')

    if dafny_input.CreationDate.is_Some:
        output["CreationDate"] = datetime.fromisoformat(dafny_input.CreationDate.value.VerbatimString(False))

    if dafny_input.LastUpdatedDate.is_Some:
        output["LastUpdatedDate"] = datetime.fromisoformat(dafny_input.LastUpdatedDate.value.VerbatimString(False))
...
def com_amazonaws_kms_ListAliasesResponse(dafny_input):
    ...
    if dafny_input.Aliases.is_Some:
        output["Aliases"] = [aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_AliasListEntry(list_element) for list_element in dafny_input.Aliases.value]
...
def com_amazonaws_kms_ListGrantsRequest(dafny_input):
    ...
    if dafny_input.GranteePrincipal.is_Some:
        output["GranteePrincipal"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.GranteePrincipal.value).decode('utf-16-be')
...
def com_amazonaws_kms_GrantListEntry(dafny_input):
    ...
    if dafny_input.IssuingAccount.is_Some:
        output["IssuingAccount"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.IssuingAccount.value).decode('utf-16-be')

    if dafny_input.Operations.is_Some:
        output["Operations"] = [aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_GrantOperation(list_element) for list_element in dafny_input.Operations.value]
...
def com_amazonaws_kms_ListGrantsResponse(dafny_input):
    ...
    if dafny_input.Grants.is_Some:
        output["Grants"] = [aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_GrantListEntry(list_element) for list_element in dafny_input.Grants.value]
...
def com_amazonaws_kms_ListKeyPoliciesResponse(dafny_input):
    ...
    if dafny_input.PolicyNames.is_Some:
        output["PolicyNames"] = [b''.join(ord(c).to_bytes(2, 'big') for c in list_element).decode('utf-16-be') for list_element in dafny_input.PolicyNames.value]
...
def com_amazonaws_kms_RotationsListEntry(dafny_input):
    ...
    if dafny_input.RotationDate.is_Some:
        output["RotationDate"] = datetime.fromisoformat(dafny_input.RotationDate.value.VerbatimString(False))

    if dafny_input.RotationType.is_Some:
        output["RotationType"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_RotationType(dafny_input.RotationType.value)
...
def com_amazonaws_kms_ListKeyRotationsResponse(dafny_input):
    ...
    if dafny_input.Rotations.is_Some:
        output["Rotations"] = [aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_RotationsListEntry(list_element) for list_element in dafny_input.Rotations.value]
...
def com_amazonaws_kms_KeyListEntry(dafny_input):
    ...
    if dafny_input.KeyArn.is_Some:
        output["KeyArn"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.KeyArn.value).decode('utf-16-be')
...
def com_amazonaws_kms_ListKeysResponse(dafny_input):
    ...
    if dafny_input.Keys.is_Some:
        output["Keys"] = [aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_KeyListEntry(list_element) for list_element in dafny_input.Keys.value]
...
def com_amazonaws_kms_ReEncryptRequest(dafny_input):
    ...
    if dafny_input.SourceEncryptionContext.is_Some:
        output["SourceEncryptionContext"] = {b''.join(ord(c).to_bytes(2, 'big') for c in key).decode('utf-16-be'): b''.join(ord(c).to_bytes(2, 'big') for c in value).decode('utf-16-be') for (key, value) in dafny_input.SourceEncryptionContext.value.items }

    if dafny_input.SourceKeyId.is_Some:
        output["SourceKeyId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.SourceKeyId.value).decode('utf-16-be')

    output["DestinationKeyId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.DestinationKeyId).decode('utf-16-be')
    if dafny_input.DestinationEncryptionContext.is_Some:
        output["DestinationEncryptionContext"] = {b''.join(ord(c).to_bytes(2, 'big') for c in key).decode('utf-16-be'): b''.join(ord(c).to_bytes(2, 'big') for c in value).decode('utf-16-be') for (key, value) in dafny_input.DestinationEncryptionContext.value.items }

    if dafny_input.SourceEncryptionAlgorithm.is_Some:
        output["SourceEncryptionAlgorithm"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_EncryptionAlgorithmSpec(dafny_input.SourceEncryptionAlgorithm.value)

    if dafny_input.DestinationEncryptionAlgorithm.is_Some:
        output["DestinationEncryptionAlgorithm"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_EncryptionAlgorithmSpec(dafny_input.DestinationEncryptionAlgorithm.value)
...
def com_amazonaws_kms_ReplicateKeyRequest(dafny_input):
    output = {}
    output["KeyId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.KeyId).decode('utf-16-be')
    output["ReplicaRegion"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.ReplicaRegion).decode('utf-16-be')
    if dafny_input.Policy.is_Some:
        output["Policy"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.Policy.value).decode('utf-16-be')
...
def com_amazonaws_kms_ReplicateKeyResponse(dafny_input):
    ...
    if dafny_input.ReplicaKeyMetadata.is_Some:
        output["ReplicaKeyMetadata"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_KeyMetadata(dafny_input.ReplicaKeyMetadata.value)

    if dafny_input.ReplicaPolicy.is_Some:
        output["ReplicaPolicy"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.ReplicaPolicy.value).decode('utf-16-be')

    if dafny_input.ReplicaTags.is_Some:
        output["ReplicaTags"] = [aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_Tag(list_element) for list_element in dafny_input.ReplicaTags.value]
...
def com_amazonaws_kms_RevokeGrantRequest(dafny_input):
    output = {}
    output["KeyId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.KeyId).decode('utf-16-be')
    output["GrantId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.GrantId).decode('utf-16-be')
    if dafny_input.DryRun.is_Some:
        output["DryRun"] = dafny_input.DryRun.value
...
def com_amazonaws_kms_SignRequest(dafny_input):
    ...
    if dafny_input.MessageType.is_Some:
        output["MessageType"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_MessageType(dafny_input.MessageType.value)

    if dafny_input.GrantTokens.is_Some:
        output["GrantTokens"] = [b''.join(ord(c).to_bytes(2, 'big') for c in list_element).decode('utf-16-be') for list_element in dafny_input.GrantTokens.value]

    output["SigningAlgorithm"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_SigningAlgorithmSpec(dafny_input.SigningAlgorithm)
    if dafny_input.DryRun.is_Some:
        output["DryRun"] = dafny_input.DryRun.value
...
def com_amazonaws_kms_SignResponse(dafny_input):
    ...
    if dafny_input.SigningAlgorithm.is_Some:
        output["SigningAlgorithm"] = aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_SigningAlgorithmSpec(dafny_input.SigningAlgorithm.value)
...
def com_amazonaws_kms_TagResourceRequest(dafny_input):
    output = {}
    output["KeyId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.KeyId).decode('utf-16-be')
    output["Tags"] = [aws_cryptography_internal_kms.smithygenerated.com_amazonaws_kms.dafny_to_aws_sdk.com_amazonaws_kms_Tag(list_element) for list_element in dafny_input.Tags]
    return output
...
def com_amazonaws_kms_UpdateCustomKeyStoreRequest(dafny_input):
    ...
    if dafny_input.NewCustomKeyStoreName.is_Some:
        output["NewCustomKeyStoreName"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.NewCustomKeyStoreName.value).decode('utf-16-be')
...
def com_amazonaws_kms_UpdateKeyDescriptionRequest(dafny_input):
    output = {}
    output["KeyId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.KeyId).decode('utf-16-be')
    output["Description"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.Description).decode('utf-16-be')
    return output

def com_amazonaws_kms_UpdatePrimaryRegionRequest(dafny_input):
    output = {}
    output["KeyId"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.KeyId).decode('utf-16-be')
    output["PrimaryRegion"] = b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.PrimaryRegion).decode('utf-16-be')
    return output
