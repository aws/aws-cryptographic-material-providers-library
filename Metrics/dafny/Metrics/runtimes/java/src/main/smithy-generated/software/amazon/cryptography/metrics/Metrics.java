...
public class Metrics {
  ...
  public CreateMetricsAgentOutput CreateTextMetricsAgent(
    CreateTextMetricsAgentInput input
  ) {
    software.amazon.cryptography.metrics.internaldafny.types.CreateTextMetricsAgentInput dafnyValue =
      ToDafny.CreateTextMetricsAgentInput(input);
    Result<
      software.amazon.cryptography.metrics.internaldafny.types.CreateMetricsAgentOutput,
      Error
    > result = this._impl.CreateTextMetricsAgent(dafnyValue);
    if (result.is_Failure()) {
      throw ToNative.Error(result.dtor_error());
    }
    return ToNative.CreateMetricsAgentOutput(result.dtor_value());
