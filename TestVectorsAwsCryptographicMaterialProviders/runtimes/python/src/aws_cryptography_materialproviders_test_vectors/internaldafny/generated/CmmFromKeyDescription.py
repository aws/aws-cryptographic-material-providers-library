...
class default__:
    ...
    def ToCmm(mpl, keys, description, forOperation):
        ...
        d_0_material_ = KeyringFromKeyDescription.default__.GetKeyMaterial(keys, description)
        source0_ = description
        with _dafny.label("match0"):
            if True:
                if source0_.is_RequiredEncryptionContext:
                    d_1_cmm_ = source0_.RequiredEncryptionContext
                    if True:
                        d_2_valueOrError0_: Wrappers.Result = None
                        out0_: Wrappers.Result
                        out0_ = default__.ToCmm(mpl, keys, (d_1_cmm_).underlying, forOperation)
                        d_2_valueOrError0_ = out0_
                        if (d_2_valueOrError0_).IsFailure():
                            output = (d_2_valueOrError0_).PropagateFailure()
                            return output
                        d_3_underlying_: AwsCryptographyMaterialProvidersTypes.ICryptographicMaterialsManager
                        d_3_underlying_ = (d_2_valueOrError0_).Extract()
                        d_4_output_k_: Wrappers.Result
                        out1_: Wrappers.Result
                        out1_ = (mpl).CreateRequiredEncryptionContextCMM(AwsCryptographyMaterialProvidersTypes.CreateRequiredEncryptionContextCMMInput_CreateRequiredEncryptionContextCMMInput(Wrappers.Option_Some(d_3_underlying_), Wrappers.Option_None(), (d_1_cmm_).requiredEncryptionContextKeys))
                        d_4_output_k_ = out1_
                        def lambda0_(d_5_e_):
                            return AwsCryptographyMaterialProvidersTestVectorKeysTypes.Error_AwsCryptographyMaterialProviders(d_5_e_)

                        output = (d_4_output_k_).MapFailure(lambda0_)
                    raise _dafny.Break("match0")
            if True:
                if True:
                    d_6_valueOrError1_: Wrappers.Result = None
                    out2_: Wrappers.Result
                    out2_ = KeyringFromKeyDescription.default__.ToKeyring(mpl, keys, description)
                    ...
                    d_7_keyring_ = (d_6_valueOrError1_).Extract()
                    d_8_output_k_: Wrappers.Result
                    out3_: Wrappers.Result
                    out3_ = (mpl).CreateDefaultCryptographicMaterialsManager(AwsCryptographyMaterialProvidersTypes.CreateDefaultCryptographicMaterialsManagerInput_CreateDefaultCryptographicMaterialsManagerInput(d_7_keyring_))
                    d_8_output_k_ = out3_
                    def lambda1_(d_9_e_):
                        return AwsCryptographyMaterialProvidersTestVectorKeysTypes.Error_AwsCryptographyMaterialProviders(d_9_e_)

                    output = (d_8_output_k_).MapFailure(lambda1_)
            pass
        return output
