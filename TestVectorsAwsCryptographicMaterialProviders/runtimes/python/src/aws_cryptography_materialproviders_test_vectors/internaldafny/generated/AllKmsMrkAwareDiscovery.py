...
class default__:
    ...
    def AllDiscoveryFilters(instance):
        return _dafny.Set({Wrappers.Option_Some(AwsCryptographyMaterialProvidersTypes.DiscoveryFilter_DiscoveryFilter(_dafny.Seq([_dafny.Seq("658956600833")]), _dafny.Seq("aws"))), Wrappers.Option_None()})
    @_dafny.classproperty
    def KeyDescriptions(instance):
        def iife0_():
            coll0_ = _dafny.Set()
            compr_0_: Wrappers.Option
            for compr_0_ in (default__.AllDiscoveryFilters).Elements:
                d_0_filter_: Wrappers.Option = compr_0_
                if (d_0_filter_) in (default__.AllDiscoveryFilters):
                    coll0_ = coll0_.union(_dafny.Set([AwsCryptographyMaterialProvidersTestVectorKeysTypes.KeyDescription_KmsMrkDiscovery(AwsCryptographyMaterialProvidersTestVectorKeysTypes.KmsMrkAwareDiscovery_KmsMrkAwareDiscovery(_dafny.Seq(""), _dafny.Seq("us-west-2"), d_0_filter_))]))
    ...
    def Tests(instance):
        def iife0_():
            ...
            for compr_0_ in (AllKmsMrkAware.default__.KeyDescriptions).Elements:
                d_0_encryptDescription_: AwsCryptographyMaterialProvidersTestVectorKeysTypes.KeyDescription = compr_0_
                if (d_0_encryptDescription_) in (AllKmsMrkAware.default__.KeyDescriptions):
                    compr_1_: AwsCryptographyMaterialProvidersTestVectorKeysTypes.KeyDescription
                    for compr_1_ in (default__.KeyDescriptions).Elements:
                        d_1_decryptDescription_: AwsCryptographyMaterialProvidersTestVectorKeysTypes.KeyDescription = compr_1_
                        if (d_1_decryptDescription_) in (default__.KeyDescriptions):
                                ...
                                if (d_2_algorithmSuite_) in (AllAlgorithmSuites.default__.AllAlgorithmSuites):
                                    ...
                                    for compr_3_ in [AllAlgorithmSuites.default__.GetCompatibleCommitmentPolicy(d_2_algorithmSuite_)]:
                                        ...
                                        if (d_3_commitmentPolicy_) == (AllAlgorithmSuites.default__.GetCompatibleCommitmentPolicy(d_2_algorithmSuite_)):
                                            coll0_ = coll0_.union(_dafny.Set([TestVectors.EncryptTestVector_PositiveEncryptKeyringVector((((_dafny.Seq("Generated Discovery KMS MRK ")) + (((d_0_encryptDescription_).KmsMrk).keyId)) + (_dafny.Seq("->"))) + (((((_dafny.Seq("Filter ")) + (((((d_1_decryptDescription_).KmsMrkDiscovery).awsKmsDiscoveryFilter).value).partition)) + (_dafny.Seq(" "))) + (Seq.default__.Flatten(((((d_1_decryptDescription_).KmsMrkDiscovery).awsKmsDiscoveryFilter).value).accountIds)) if (((d_1_decryptDescription_).KmsMrkDiscovery).awsKmsDiscoveryFilter).is_Some else _dafny.Seq("No Filter"))), Wrappers.Option_None(), _dafny.Map({}), d_3_commitmentPolicy_, d_2_algorithmSuite_, Wrappers.Option_None(), Wrappers.Option_None(), d_0_encryptDescription_, d_1_decryptDescription_, Wrappers.Option_None())]))
            return _dafny.Set(coll0_)
        return iife0_()
