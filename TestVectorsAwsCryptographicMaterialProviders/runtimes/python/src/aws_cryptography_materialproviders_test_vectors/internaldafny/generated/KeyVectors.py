...
class default__:
    ...
    def KeyVectors(config):
        ...
        if not(not((d_0_valueOrError0_).IsFailure())):
            raise _dafny.HaltException("dafny/KeyVectors/src/Index.dfy(30,26): " + _dafny.string_of(d_0_valueOrError0_))
        d_1_keysManifestBv_: _dafny.Seq
        d_1_keysManifestBv_ = (d_0_valueOrError0_).Extract()
        d_2_keysManifestBytes_: _dafny.Seq
        d_2_keysManifestBytes_ = JSONHelpers.default__.BvToBytes(d_1_keysManifestBv_)
        d_3_valueOrError1_: Wrappers.Result = Wrappers.Result.default(JSON_Values.JSON.default())()
        def lambda0_(d_4_e_):
            return AwsCryptographyMaterialProvidersTestVectorKeysTypes.Error_KeyVectorException((d_4_e_).ToString())

        d_3_valueOrError1_ = (JSON_API.default__.Deserialize(d_2_keysManifestBytes_)).MapFailure(lambda0_)
        if (d_3_valueOrError1_).IsFailure():
            res = (d_3_valueOrError1_).PropagateFailure()
            return res
        d_5_keysManifestJSON_: JSON_Values.JSON
        d_5_keysManifestJSON_ = (d_3_valueOrError1_).Extract()
        if not((d_5_keysManifestJSON_).is_Object):
            raise _dafny.HaltException("dafny/KeyVectors/src/Index.dfy(36,4): " + _dafny.string_of(_dafny.Seq("expectation violation")))
        d_6_valueOrError2_: Wrappers.Result = Wrappers.Result.default(JSON_Values.JSON.default())()
        d_6_valueOrError2_ = JSONHelpers.default__.Get(_dafny.Seq("keys"), (d_5_keysManifestJSON_).obj)
        if not(not((d_6_valueOrError2_).IsFailure())):
            raise _dafny.HaltException("dafny/KeyVectors/src/Index.dfy(37,22): " + _dafny.string_of(d_6_valueOrError2_))
        d_7_keysObject_: JSON_Values.JSON
        d_7_keysObject_ = (d_6_valueOrError2_).Extract()
        if not((d_7_keysObject_).is_Object):
            raise _dafny.HaltException("dafny/KeyVectors/src/Index.dfy(38,4): " + _dafny.string_of(_dafny.Seq("expectation violation")))
        d_8_maybeMpl_: Wrappers.Result
        ...
        def lambda1_(d_10_e_):
            return AwsCryptographyMaterialProvidersTestVectorKeysTypes.Error_AwsCryptographyMaterialProviders(d_10_e_)

        d_9_valueOrError3_ = (d_8_maybeMpl_).MapFailure(lambda1_)
        if (d_9_valueOrError3_).IsFailure():
            res = (d_9_valueOrError3_).PropagateFailure()
            return res
        d_11_mpl_: MaterialProviders.MaterialProvidersClient
        d_11_mpl_ = (d_9_valueOrError3_).Extract()
        d_12_valueOrError4_: Wrappers.Result = Wrappers.Result.default(_dafny.Map)()
        def lambda2_(d_13_e_):
            return AwsCryptographyMaterialProvidersTestVectorKeysTypes.Error_KeyVectorException(d_13_e_)

        d_12_valueOrError4_ = (KeyMaterial.default__.BuildKeyMaterials(d_11_mpl_, (d_7_keysObject_).obj)).MapFailure(lambda2_)
        if (d_12_valueOrError4_).IsFailure():
            res = (d_12_valueOrError4_).PropagateFailure()
            return res
        d_14_keys_: _dafny.Map
        d_14_keys_ = (d_12_valueOrError4_).Extract()
        d_15_config_: KeysVectorOperations.Config
        d_15_config_ = KeysVectorOperations.Config_Config(d_14_keys_, d_5_keysManifestJSON_)
        d_16_client_: KeyVectorsClient
        nw0_ = KeyVectorsClient()
        nw0_.ctor__(d_15_config_)
        d_16_client_ = nw0_
        res = Wrappers.Result_Success(d_16_client_)
        return res
