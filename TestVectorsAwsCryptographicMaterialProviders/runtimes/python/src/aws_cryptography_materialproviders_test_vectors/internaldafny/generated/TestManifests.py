...
class default__:
    ...
    def TestEncrypts(tests, keys):
        ...
        d_1_decryptableTests_ = _dafny.Seq([])
        d_2_time_: Time.AbsoluteTime
        out0_: Time.AbsoluteTime
        out0_ = Time.default__.GetAbsoluteTime()
        d_2_time_ = out0_
        hi0_ = len(tests)
        for d_3_i_ in range(0, hi0_):
            d_4_test_: TestVectors.EncryptTest
            d_4_test_ = (tests)[d_3_i_]
            d_5_pass_: bool
            d_6_maybeEncryptionMaterials_: Wrappers.Option
            out1_: bool
            out2_: Wrappers.Option
            out1_, out2_ = TestVectors.default__.TestGetEncryptionMaterials(d_4_test_)
            d_5_pass_ = out1_
            d_6_maybeEncryptionMaterials_ = out2_
            if (d_5_pass_) and (not(((d_4_test_).vector).is_NegativeEncryptKeyringVector)):
                d_1_decryptableTests_ = (d_1_decryptableTests_) + (_dafny.Seq([(d_4_test_, (d_6_maybeEncryptionMaterials_).value)]))
            elif not(d_5_pass_):
                d_0_hasFailure_ = True
        d_7_elapsed_: Time.RelativeTime
        out3_: Time.RelativeTime
        out3_ = Time.default__.TimeSince(d_2_time_)
        d_7_elapsed_ = out3_
        Time.default__.PrintTimeLong(d_7_elapsed_, _dafny.Seq("TestEncrypts"), Wrappers.Option_Some(default__.LogFileName()))
        _dafny.print(_dafny.string_of(_dafny.Seq("\n=================== Completed ")))
        _dafny.print(_dafny.string_of(len(tests)))
        _dafny.print(_dafny.string_of(_dafny.Seq(" Encrypt Tests =================== \n\n")))
        if not(not(d_0_hasFailure_)):
            raise _dafny.HaltException("dafny/TestVectorsAwsCryptographicMaterialProviders/src/TestManifests.dfy(90,4): " + _dafny.string_of(_dafny.Seq("expectation violation")))
        d_8_valueOrError0_: Wrappers.Result = Wrappers.Result.default(_dafny.Seq)()
        out4_: Wrappers.Result
        out4_ = default__.ToDecryptTestVectors(keys, d_1_decryptableTests_)
        d_8_valueOrError0_ = out4_
        if not(not((d_8_valueOrError0_).IsFailure())):
            raise _dafny.HaltException("dafny/TestVectorsAwsCryptographicMaterialProviders/src/TestManifests.dfy(91,14): " + _dafny.string_of(d_8_valueOrError0_))
        output = (d_8_valueOrError0_).Extract()
        return output
    ...
    def TestDecrypts(tests):
        ...
        for d_2_i_ in range(0, hi0_):
            d_3_pass_: bool
            out1_: bool
            out1_ = TestVectors.default__.TestDecryptMaterials((tests)[d_2_i_])
            d_3_pass_ = out1_
            if not(d_3_pass_):
                d_0_hasFailure_ = True
        d_4_elapsed_: Time.RelativeTime
        out2_: Time.RelativeTime
        out2_ = Time.default__.TimeSince(d_1_time_)
        d_4_elapsed_ = out2_
        Time.default__.PrintTimeLong(d_4_elapsed_, _dafny.Seq("TestDecrypts"), Wrappers.Option_Some(default__.LogFileName()))
        _dafny.print(_dafny.string_of(_dafny.Seq("\n=================== Completed ")))
        _dafny.print(_dafny.string_of(len(tests)))
        _dafny.print(_dafny.string_of(_dafny.Seq(" Decrypt Tests =================== \n\n")))
        if not(not(d_0_hasFailure_)):
            raise _dafny.HaltException("dafny/TestVectorsAwsCryptographicMaterialProviders/src/TestManifests.dfy(134,4): " + _dafny.string_of(_dafny.Seq("expectation violation")))
    ...
    def GetManifest(manifestPath, manifestFileName):
        ...
        if not(not((d_16_valueOrError8_).IsFailure())):
            raise _dafny.HaltException("dafny/TestVectorsAwsCryptographicMaterialProviders/src/TestManifests.dfy(242,16): " + _dafny.string_of(d_16_valueOrError8_))
