...
class default__:
        ...
        while True:
            with _dafny.label():
                if (i) == (len(keys)):
                    ...
                    d_0___accumulator_ = (d_0___accumulator_) + (_dafny.Seq([AwsCryptographyMaterialProvidersTestVectorKeysTypes.KeyDescription_AES(AwsCryptographyMaterialProvidersTestVectorKeysTypes.RawAES_RawAES((keys)[i], (_dafny.Seq("aws-raw-vectors-persistent-")) + ((keys)[i])))]))
                    in3_ = keys
                    in4_ = key
                    in5_ = (i) + (1)
                    keys = in3_
    ...
    def KeyDescriptionsGeneratorAndChildren(instance):
        def iife0_():
            coll0_ = _dafny.Set()
            compr_0_: _dafny.Seq
            for compr_0_ in (AllRawAES.default__.aesPersistentKeyNames).Elements:
                d_0_key_: _dafny.Seq = compr_0_
                if (d_0_key_) in (AllRawAES.default__.aesPersistentKeyNames):
                    coll0_ = coll0_.union(_dafny.Set([AwsCryptographyMaterialProvidersTestVectorKeysTypes.KeyDescription_Multi(AwsCryptographyMaterialProvidersTestVectorKeysTypes.MultiKeyring_MultiKeyring(Wrappers.Option_Some(AwsCryptographyMaterialProvidersTestVectorKeysTypes.KeyDescription_AES(AwsCryptographyMaterialProvidersTestVectorKeysTypes.RawAES_RawAES(d_0_key_, (_dafny.Seq("aws-raw-vectors-persistent-")) + (d_0_key_)))), default__.getChildKeyrings(AllRawAES.default__.aesPersistentKeyNames, d_0_key_, 0)))]))
    ...
    def OnlyGeneratorKeyDescriptions(instance):
        def iife0_():
            ...
            for compr_0_ in (AllRawAES.default__.aesPersistentKeyNames).Elements:
                d_0_key_: _dafny.Seq = compr_0_
                if (d_0_key_) in (AllRawAES.default__.aesPersistentKeyNames):
                    coll0_ = coll0_.union(_dafny.Set([AwsCryptographyMaterialProvidersTestVectorKeysTypes.KeyDescription_Multi(AwsCryptographyMaterialProvidersTestVectorKeysTypes.MultiKeyring_MultiKeyring(Wrappers.Option_Some(AwsCryptographyMaterialProvidersTestVectorKeysTypes.KeyDescription_AES(AwsCryptographyMaterialProvidersTestVectorKeysTypes.RawAES_RawAES(d_0_key_, (_dafny.Seq("aws-raw-vectors-persistent-")) + (d_0_key_)))), _dafny.Seq([])))]))
            return _dafny.Set(coll0_)
        return iife0_()
