...
class default__:
    ...
    def Main(args):
        ...
        if (d_1_parsedOptions_q_).is_Success:
            ...
            if (d_3_op_q_).is_Success:
                d_4_op_: MplManifestOptions.ManifestOptions
                ...
                with _dafny.label("match0"):
                    if True:
                        if source0_.is_Decrypt:
                            ...
                            out0_ = TestManifests.default__.StartDecrypt(d_4_op_)
                            d_5_result_ = out0_
                            if (d_5_result_).is_Failure:
                                _dafny.print(_dafny.string_of((d_5_result_).error))
                            if not((d_5_result_).is_Success):
                                raise _dafny.HaltException("dafny/TestVectorsAwsCryptographicMaterialProviders/src/Index.dfy(57,10): " + _dafny.string_of(_dafny.Seq("expectation violation")))
                            raise _dafny.Break("match0")
                    if True:
                        if source0_.is_Encrypt:
                            d_6_result_: Wrappers.Result
                            out1_: Wrappers.Result
                            out1_ = TestManifests.default__.StartEncrypt(d_4_op_)
                            d_6_result_ = out1_
                            if (d_6_result_).is_Failure:
                                _dafny.print(_dafny.string_of((d_6_result_).error))
                            if not((d_6_result_).is_Success):
                                raise _dafny.HaltException("dafny/TestVectorsAwsCryptographicMaterialProviders/src/Index.dfy(63,10): " + _dafny.string_of(_dafny.Seq("expectation violation")))
                            raise _dafny.Break("match0")
                    if True:
                        d_7_result_: Wrappers.Result
                        out2_: Wrappers.Result
                        out2_ = CompleteVectors.default__.WriteStuff(d_4_op_)
                        d_7_result_ = out2_
                        if (d_7_result_).is_Failure:
                            _dafny.print(_dafny.string_of((d_7_result_).error))
                        if not((d_7_result_).is_Success):
                            raise _dafny.HaltException("dafny/TestVectorsAwsCryptographicMaterialProviders/src/Index.dfy(69,10): " + _dafny.string_of(_dafny.Seq("expectation violation")))
                    pass
            elif True:
    ...
    def ParseDecryptCmd(params):
        ...
        d_2_manifestPath_ = ((d_0_manifestPath_q_).value if (d_0_manifestPath_q_).is_Some else _dafny.Seq("."))
        d_3_valueOrError0_ = Wrappers.default__.Need((0) < (len(d_2_manifestPath_)), _dafny.Seq("Invalid manifest path length\n"))
        if (d_3_valueOrError0_).IsFailure():
            return (d_3_valueOrError0_).PropagateFailure()
        elif True:
            return Wrappers.Result_Success(MplManifestOptions.ManifestOptions_Decrypt((d_2_manifestPath_ if (Seq.default__.Last(d_2_manifestPath_)) == ('/') else (d_2_manifestPath_) + (_dafny.Seq("/"))), (d_1_testName_q_ if (d_1_testName_q_).is_Some else Wrappers.Option_None())))

    @staticmethod
    def ParseEncryptCmd(params):
        d_0_manifestPath_q_ = GetOpt.default__.OptValue(params, _dafny.Seq("manifest-path"))
        d_1_decryptManifestPath_q_ = GetOpt.default__.OptValue(params, _dafny.Seq("decrypt-manifest-path"))
        d_2_testName_q_ = GetOpt.default__.OptValue(params, _dafny.Seq("test-name"))
        d_3_manifestPath_ = ((d_0_manifestPath_q_).value if (d_0_manifestPath_q_).is_Some else _dafny.Seq("."))
        d_4_decryptManifestPath_ = ((d_1_decryptManifestPath_q_).value if (d_1_decryptManifestPath_q_).is_Some else _dafny.Seq("."))
        d_5_valueOrError0_ = Wrappers.default__.Need(((0) < (len(d_3_manifestPath_))) and ((0) < (len(d_4_decryptManifestPath_))), _dafny.Seq("Invalid manifest or decrypt manifest path length\n"))
        if (d_5_valueOrError0_).IsFailure():
            return (d_5_valueOrError0_).PropagateFailure()
        elif True:
            return Wrappers.Result_Success(MplManifestOptions.ManifestOptions_Encrypt((d_3_manifestPath_ if (Seq.default__.Last(d_3_manifestPath_)) == ('/') else (d_3_manifestPath_) + (_dafny.Seq("/"))), (d_4_decryptManifestPath_ if (Seq.default__.Last(d_4_decryptManifestPath_)) == ('/') else (d_4_decryptManifestPath_) + (_dafny.Seq("/"))), (d_2_testName_q_ if (d_2_testName_q_).is_Some else Wrappers.Option_None())))
    ...
    def ParseEncryptManifestCmd(params):
        ...
        if (d_2_valueOrError0_).IsFailure():
            ...
            return Wrappers.Result_Success(MplManifestOptions.ManifestOptions_EncryptManifest((d_1_encryptManifestOutput_ if (Seq.default__.Last(d_1_encryptManifestOutput_)) == ('/') else (d_1_encryptManifestOutput_) + (_dafny.Seq("/")))))
