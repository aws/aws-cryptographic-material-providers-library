                    ...
                    if True:
                        ...
                        if (d_24_valueOrError5_).IsFailure():
                            ...
                            if (d_26_valueOrError6_).IsFailure():
                                ...
                                d_28_valueOrError7_ = default__.KeyDescriptionListToJson((d_15_MultiKeyring_).childKeyrings, outputVersion)
                                if (d_28_valueOrError7_).IsFailure():
                                    return (d_28_valueOrError7_).PropagateFailure()
                                elif True:
                                    d_29_keyrings_ = (d_28_valueOrError7_).Extract()
                                    return Wrappers.Result_Success(JSON_Values.JSON_Array(d_29_keyrings_))
        if True:
            d_30_RequiredEncryptionContext_ = source0_.RequiredEncryptionContext
            d_31_valueOrError8_ = default__.ToJson((d_30_RequiredEncryptionContext_).underlying, outputVersion)
            if (d_31_valueOrError8_).IsFailure():
                return (d_31_valueOrError8_).PropagateFailure()
            elif True:
                d_32_underlying_ = (d_31_valueOrError8_).Extract()
                def lambda3_(d_34_key_):
                    def iife4_(_pat_let4_0):
                        def iife5_(d_35_valueOrError10_):
                            def iife6_(_pat_let5_0):
                                def iife7_(d_36_s_):
                                    return Wrappers.Result_Success(JSON_Values.JSON_String(d_36_s_))
                                return iife7_(_pat_let5_0)
                            return ((d_35_valueOrError10_).PropagateFailure() if (d_35_valueOrError10_).IsFailure() else iife6_((d_35_valueOrError10_).Extract()))
                        return iife5_(_pat_let4_0)
                    return iife4_(UTF8.default__.Decode(d_34_key_))

                d_33_valueOrError9_ = Seq.default__.MapWithResult(lambda3_, (d_30_RequiredEncryptionContext_).requiredEncryptionContextKeys)
                if (d_33_valueOrError9_).IsFailure():
                    return (d_33_valueOrError9_).PropagateFailure()
                elif True:
                    d_37_requiredEncryptionContextKeys_ = (d_33_valueOrError9_).Extract()
                    return Wrappers.Result_Success(JSON_Values.JSON_Object(_dafny.Seq([(_dafny.Seq("type"), JSON_Values.JSON_String(_dafny.Seq("required-encryption-context-cmm"))), (_dafny.Seq("underlying"), d_32_underlying_), (_dafny.Seq("requiredEncryptionContextKeys"), JSON_Values.JSON_Array(d_37_requiredEncryptionContextKeys_))])))
