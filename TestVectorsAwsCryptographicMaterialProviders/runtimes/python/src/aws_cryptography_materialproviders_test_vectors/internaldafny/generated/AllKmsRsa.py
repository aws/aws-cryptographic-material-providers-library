...
class default__:
    ...
    def KeyDescriptions(instance):
        def iife0_():
            def iife2_():
                coll2_ = _dafny.Set()
                compr_3_: ComAmazonawsKmsTypes.EncryptionAlgorithmSpec
                for compr_3_ in ComAmazonawsKmsTypes.EncryptionAlgorithmSpec.AllSingletonConstructors:
                    d_3_e_: ComAmazonawsKmsTypes.EncryptionAlgorithmSpec = compr_3_
                    if not((d_3_e_).is_SYMMETRIC__DEFAULT):
                        coll2_ = coll2_.union(_dafny.Set([d_3_e_]))
                return _dafny.Set(coll2_)
            coll0_ = _dafny.Set()
            compr_0_: _dafny.Seq
            for compr_0_ in (default__.AllKmsRsaKeys).Elements:
                d_0_key_: _dafny.Seq = compr_0_
                if (d_0_key_) in (default__.AllKmsRsaKeys):
                    def iife1_():
                        coll1_ = _dafny.Set()
                        compr_2_: ComAmazonawsKmsTypes.EncryptionAlgorithmSpec
                        for compr_2_ in ComAmazonawsKmsTypes.EncryptionAlgorithmSpec.AllSingletonConstructors:
                            d_1_e_: ComAmazonawsKmsTypes.EncryptionAlgorithmSpec = compr_2_
                            if not((d_1_e_).is_SYMMETRIC__DEFAULT):
                                coll1_ = coll1_.union(_dafny.Set([d_1_e_]))
                        return _dafny.Set(coll1_)
                    compr_1_: ComAmazonawsKmsTypes.EncryptionAlgorithmSpec
                    for compr_1_ in (iife1_()
                    ).Elements:
                        d_2_encryptionAlgorithm_: ComAmazonawsKmsTypes.EncryptionAlgorithmSpec = compr_1_
                        if (d_2_encryptionAlgorithm_) in (iife2_()
                        ):
                            coll0_ = coll0_.union(_dafny.Set([AwsCryptographyMaterialProvidersTestVectorKeysTypes.KeyDescription_KmsRsa(AwsCryptographyMaterialProvidersTestVectorKeysTypes.KmsRsaKeyring_KmsRsaKeyring(d_0_key_, d_2_encryptionAlgorithm_))]))
        ...
        def iife0_():
            ...
            for compr_0_ in (AllAlgorithmSuites.default__.AllAlgorithmSuites).Elements:
                ...
                if AlgorithmSuites.AlgorithmSuite._Is(d_0_suite_):
                    if ((d_0_suite_) in (AllAlgorithmSuites.default__.AllAlgorithmSuites)) and (not(((d_0_suite_).signature).is_ECDSA)):
        ...
        def iife0_():
            ...
            for compr_0_ in (default__.KeyDescriptions).Elements:
                ...
                if (d_0_keyDescription_) in (default__.KeyDescriptions):
                    ...
                    for compr_1_ in (default__.algorithmSuites).Elements:
                        d_1_algorithmSuite_: AwsCryptographyMaterialProvidersTypes.AlgorithmSuiteInfo = compr_1_
                        if (d_1_algorithmSuite_) in (default__.algorithmSuites):
