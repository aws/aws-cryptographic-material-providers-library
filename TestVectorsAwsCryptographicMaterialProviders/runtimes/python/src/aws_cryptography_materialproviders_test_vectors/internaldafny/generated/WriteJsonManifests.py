...
class default__:
    ...
    def EncryptionContextToJson(key, m):
        d_0_keys_ = SortedSets.default__.SetToSequence((m).keys)
        def lambda0_(d_2_m_):
            def lambda1_(d_3_k_):
                def iife0_(_pat_let18_0):
                    def iife1_(d_4_valueOrError1_):
                        def iife2_(_pat_let19_0):
                            def iife3_(d_5_key_):
                                def iife4_(_pat_let20_0):
                                    def iife5_(d_6_valueOrError2_):
                                        def iife6_(_pat_let21_0):
                                            def iife7_(d_7_value_):
                                                return Wrappers.Result_Success((d_5_key_, JSON_Values.JSON_String(d_7_value_)))
                                            return iife7_(_pat_let21_0)
                                        return ((d_6_valueOrError2_).PropagateFailure() if (d_6_valueOrError2_).IsFailure() else iife6_((d_6_valueOrError2_).Extract()))
                                    return iife5_(_pat_let20_0)
                                return iife4_(UTF8.default__.Decode((d_2_m_)[d_3_k_]))
                            return iife3_(_pat_let19_0)
                        return ((d_4_valueOrError1_).PropagateFailure() if (d_4_valueOrError1_).IsFailure() else iife2_((d_4_valueOrError1_).Extract()))
                    return iife1_(_pat_let18_0)
                return iife0_(UTF8.default__.Decode(d_3_k_))

            return lambda1_

        d_1_valueOrError0_ = Seq.default__.MapWithResult(lambda0_(m), d_0_keys_)
        if (d_1_valueOrError0_).IsFailure():
            return (d_1_valueOrError0_).PropagateFailure()
        elif True:
            d_8_pairsBytes_ = (d_1_valueOrError0_).Extract()
            return Wrappers.Result_Success(_dafny.Seq([(key, JSON_Values.JSON_Object(d_8_pairsBytes_))]))
