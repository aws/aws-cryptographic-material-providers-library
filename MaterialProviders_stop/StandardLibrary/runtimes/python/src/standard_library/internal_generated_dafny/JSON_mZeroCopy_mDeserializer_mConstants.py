...
assert "JSON_mZeroCopy_mDeserializer_mConstants" == __name__
JSON_mZeroCopy_mDeserializer_mConstants = sys.modules[__name__]
...
class default__:
    ...
    def Constant(cs, expected):
        d_756_valueOrError0_ = (cs).AssertBytes(expected, 0)
        if (d_756_valueOrError0_).IsFailure():
            return (d_756_valueOrError0_).PropagateFailure()
        elif True:
            d_757_cs_ = (d_756_valueOrError0_).Extract()
            return Wrappers.Result_Success((d_757_cs_).Split())
