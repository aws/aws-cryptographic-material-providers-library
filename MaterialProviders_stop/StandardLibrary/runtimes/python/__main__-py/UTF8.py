...
class default__:
    ...
    def IsASCIIString(s):
        def lambda7_(forall_var_2_):
            d_69_i_: int = forall_var_2_
            return not (((0) <= (d_69_i_)) and ((d_69_i_) < (len(s)))) or ((ord((s)[d_69_i_])) < (128))

        return _dafny.quantifier(_dafny.IntegerRange(0, len(s)), True, lambda7_)

    @staticmethod
    def EncodeAscii(s):
        d_70___accumulator_ = _dafny.Seq([])
        while True:
            with _dafny.label():
                if (len(s)) == (0):
                    return (d_70___accumulator_) + (_dafny.Seq([]))
                elif True:
                    d_71_x_ = _dafny.Seq([ord((s)[0])])
                    d_70___accumulator_ = (d_70___accumulator_) + (d_71_x_)
        ...
        while True:
            with _dafny.label():
                if (lo) == (hi):
                    ...
                    d_72_r_ = _dafny.Seq((a)[lo:hi:])
                    if UTF8.default__.Uses1Byte(d_72_r_):
                        in12_ = a
                        in13_ = (lo) + (1)
                        in14_ = hi
                        a = in12_
                        ...
                        hi = in14_
                        raise _dafny.TailCall()
                    elif ((2) <= (len(d_72_r_))) and (UTF8.default__.Uses2Bytes(d_72_r_)):
                        in15_ = a
                        in16_ = (lo) + (2)
                        in17_ = hi
                        a = in15_
                        ...
                        hi = in17_
                        raise _dafny.TailCall()
                    elif ((3) <= (len(d_72_r_))) and (UTF8.default__.Uses3Bytes(d_72_r_)):
                        in18_ = a
                        in19_ = (lo) + (3)
                        in20_ = hi
                        a = in18_
                        ...
                        hi = in20_
                        raise _dafny.TailCall()
                    elif ((4) <= (len(d_72_r_))) and (UTF8.default__.Uses4Bytes(d_72_r_)):
                        in21_ = a
                        in22_ = (lo) + (4)
                        in23_ = hi
                        a = in21_
