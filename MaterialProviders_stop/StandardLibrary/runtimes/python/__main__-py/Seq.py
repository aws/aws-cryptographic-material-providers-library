...
class default__:
    ...
    def IndexOfOption(xs, v):
        if (len(xs)) == (0):
            ...
            d_150_o_k_ = Seq.default__.IndexOfOption(_dafny.Seq((xs)[1::]), v)
    ...
    def RemoveValue(xs, v):
        if (v) not in (xs):
            ...
            d_151_i_ = Seq.default__.IndexOf(xs, v)
    ...
    def Unzip(xs):
        if (len(xs)) == (0):
            ...
            return ((d_154_a_) + (_dafny.Seq([(Seq.default__.Last(xs))[0]])), (d_155_b_) + (_dafny.Seq([(Seq.default__.Last(xs))[1]])))
        ...
        while True:
            with _dafny.label():
                if (len(xs)) == (0):
                    ...
                    d_156___accumulator_ = (_dafny.Seq([(Seq.default__.Last(xs), Seq.default__.Last(ys))])) + (d_156___accumulator_)
                    in42_ = Seq.default__.DropLast(xs)
                    in43_ = Seq.default__.DropLast(ys)
                    xs = in42_
                    ys = in43_
                    raise _dafny.TailCall()
                break
        ...
        while True:
            with _dafny.label():
                if (len(xs)) == (0):
                    ...
                    d_158___accumulator_ = (Seq.default__.Last(xs)) + (d_158___accumulator_)
                    in45_ = Seq.default__.DropLast(xs)
                    xs = in45_
                    raise _dafny.TailCall()
                break
    ...
    def MapWithResult(f, xs):
        if (len(xs)) == (0):
            ...
            if (d_160_valueOrError0_).IsFailure():
                ...
                d_162_valueOrError1_ = Seq.default__.MapWithResult(f, _dafny.Seq((xs)[1::]))
    ...
    def SetToSortedSeq(s, R):
        xs: _dafny.Seq = _dafny.Seq({})
        out4_: _dafny.Seq
        out4_ = Seq.default__.SetToSeq(s)
        xs = out4_
