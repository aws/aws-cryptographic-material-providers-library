...
impl crate::types::cryptographic_materials_manager::CryptographicMaterialsManager
    for ICryptographicMaterialsManagerDafnyWrapper
{
    ...
    > {
        let inner_input =
            crate::conversions::get_encryption_materials::_get_encryption_materials_input::to_dafny(
                input,
            );
        let inner_result =
            ::dafny_runtime::md!(self.obj.clone()).GetEncryptionMaterials(&inner_input);
        if matches!(
            inner_result.as_ref(),
