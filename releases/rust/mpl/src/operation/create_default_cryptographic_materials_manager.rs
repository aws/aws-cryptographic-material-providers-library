...
impl CreateDefaultCryptographicMaterialsManager {
    ...
    > {
        crate::validation::validate_aws_Pcryptography_PmaterialProviders_HCreateDefaultCryptographicMaterialsManagerInput_for_AwsCryptographicMaterialProviders_CreateDefaultCryptographicMaterialsManager(&input)
        ...
        let inner_result = ::dafny_runtime::md!(client.dafny_client.clone())
            .CreateDefaultCryptographicMaterialsManager(&inner_input);
        if matches!(
            inner_result.as_ref(),
            crate::r#_Wrappers_Compile::Result::Success { .. }
        ) {
            Ok(
                crate::conversions::cryptographic_materials_manager::from_dafny(
