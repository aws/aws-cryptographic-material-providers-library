...
module TestKDFK_TestVectors {
  ...
  const b6 := InternalTestVector(
                ...
                hashtype := Primitives.Types.SHA_384,
                IKM := [
  ...
  {
    var InternalTestVector(name, hashtype, IKM, info, L, OKM) := vector;
    print name + "\n";
    expect (|IKM| == 32 || |IKM| == 48) && L > 0 && 4 + |info| < INT32_MAX_LIMIT && (hashtype == Primitives.Types.SHA_256 || hashtype == Primitives.Types.SHA_384);
    expect
    ...
    TestKDF.KdfRawDeriveTest(IKM, info, L, OKM, hashtype);
  }

  method ExpectTestVector(vector : TestVector)
  {
    var TestVector(name, hashtype, IKM, info, purpose, L, OKM) := vector;
    print name + "\n";
