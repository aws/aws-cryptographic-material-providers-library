...
func (_static *CompanionStruct_Default___) ECDSASign(config Config, input m_AwsCryptographyPrimitivesTypes.ECDSASignInput) m_Wrappers.Result {
	...
	var _out0 m_Wrappers.Result
	_ = _out0
	_out0 = m_Signature.Companion_Default___.Sign((input).Dtor_signatureAlgorithm(), (input).Dtor_signingKey(), (input).Dtor_message())
	output = _out0
	return output
}
func (_static *CompanionStruct_Default___) ECDSAVerify(config Config, input m_AwsCryptographyPrimitivesTypes.ECDSAVerifyInput) m_Wrappers.Result {
	...
	_ = output
	output = m_Signature.Companion_Default___.Verify((input).Dtor_signatureAlgorithm(), (input).Dtor_verificationKey(), (input).Dtor_message(), (input).Dtor_signature())
	return output
}
