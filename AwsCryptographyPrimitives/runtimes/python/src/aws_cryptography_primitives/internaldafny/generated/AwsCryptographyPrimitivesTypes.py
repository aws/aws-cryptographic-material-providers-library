...
class Error_OpaqueWithText(Error, NamedTuple('OpaqueWithText', [('obj', Any), ('objMessage', Any)])):
    def __dafnystr__(self) -> str:
        return f'AwsCryptographyPrimitivesTypes.Error.OpaqueWithText({_dafny.string_of(self.obj)}, {_dafny.string_of(self.objMessage)})'
    def __eq__(self, __o: object) -> bool:
...
class OpaqueError:
    ...
    def _Is(source__):
        ...
        return ((d_6_e_).is_Opaque) or ((d_6_e_).is_OpaqueWithText)
...
class DummySubsetType:
    ...
    def _Is(source__):
        d_7_x_: int = source__
        return default__.IsDummySubsetType(d_7_x_)
