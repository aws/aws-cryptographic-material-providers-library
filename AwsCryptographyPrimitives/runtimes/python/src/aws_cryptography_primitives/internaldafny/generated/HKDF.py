...
class default__:
    ...
    def Expand(hmac, prk, info, expectedLength, digest):
        ...
        d_42_hashLength_: int
        d_42_hashLength_ = Digest.default__.Length(digest)
        d_43_n_: int
        d_43_n_ = _dafny.euclidian_division(((d_42_hashLength_) + (expectedLength)) - (1), d_42_hashLength_)
        (hmac).Init(prk)
        d_44_t__prev_: _dafny.Seq
        d_44_t__prev_ = _dafny.Seq([])
        d_45_t__n_: _dafny.Seq
        d_45_t__n_ = d_44_t__prev_
        d_46_i_: int
        d_46_i_ = 1
        while (d_46_i_) <= (d_43_n_):
            (hmac).BlockUpdate(d_44_t__prev_)
            (hmac).BlockUpdate(info)
            (hmac).BlockUpdate(_dafny.Seq([d_46_i_]))
            out5_: _dafny.Seq
            out5_ = (hmac).GetResult()
            d_44_t__prev_ = out5_
            d_45_t__n_ = (d_45_t__n_) + (d_44_t__prev_)
            d_46_i_ = (d_46_i_) + (1)
        okm = d_45_t__n_
