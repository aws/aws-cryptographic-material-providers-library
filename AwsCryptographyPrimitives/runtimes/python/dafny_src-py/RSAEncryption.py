...
class default__:
    ...
    def GetRSAKeyModulusLength(publicKey):
        d_95_valueOrError0_ = RSAEncryption.RSA.GetRSAKeyModulusLengthExtern(publicKey)
        if (d_95_valueOrError0_).IsFailure():
            return (d_95_valueOrError0_).PropagateFailure()
        elif True:
            d_96_length_ = (d_95_valueOrError0_).Extract()
            d_97_valueOrError1_ = Wrappers.default__.Need(((81) <= (d_96_length_)) and ((d_96_length_) < ((StandardLibrary_mUInt.default__).INT32__MAX__LIMIT)), software_amazon_cryptography_primitives_internaldafny_types.Error_AwsCryptographicPrimitivesError(_dafny.Seq("Unsupported length for RSA modulus.")))
    ...
    def Decrypt(input):
        ...
        d_98_valueOrError0_: Wrappers.Outcome = Wrappers.Outcome_Pass.default()()
    ...
    def Encrypt(input):
        output: Wrappers.Result = Wrappers.Result_Success.default(_dafny.Seq)()
        d_99_valueOrError0_: Wrappers.Outcome = Wrappers.Outcome_Pass.default()()
