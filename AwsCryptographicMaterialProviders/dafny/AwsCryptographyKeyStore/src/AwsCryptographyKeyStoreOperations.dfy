...
module AwsCryptographyKeyStoreOperations refines AbstractAwsCryptographyKeyStoreOperations {
  ...
  method GetKeyStoreInfo(config: InternalConfig)
    ...
    ensures output.Success? ==>
              && |config.storage.History.GetTableName| == |old(config.storage.History.GetTableName)| + 1
              && Seq.Last(config.storage.History.GetTableName).output.Success?
              && UTF8.Decode(Seq.Last(config.storage.History.GetTableName).output.value.Name).Success?
              && output.value.keyStoreName == UTF8.Decode(Seq.Last(config.storage.History.GetTableName).output.value.Name).value
  {
    var nameOutput :- config.storage.GetTableName(Types.GetTableNameInput);
