...
module DefaultEncryptedKeyStore {
  ...
  class {:termination false} DynamoDBEncryptedKeyStore
    ...
    predicate WriteNewKeyToStoreEnsuresPublicly(
      input: Types.WriteNewKeyToStoreInput ,
      output: Result<Types.WriteNewKeyToStoreOutput, Types.Error>
    )
    {true}
    predicate WriteNewBranchKeyVersionToKeystoreEnsuresPublicly(
      input: Types.WriteNewBranchKeyVersionToKeystoreInput ,
      output: Result<Types.WriteNewBranchKeyVersionToKeystoreOutput, Types.Error>
    )
    {true}
    ...
    predicate GetTableNameEnsuresPublicly(
      input: Types.GetTableNameInput ,
      output: Result<Types.GetTableNameOutput, Types.Error>
    )
    {true}
    ...
    method WriteNewKeyToStore' ( input: Types.WriteNewKeyToStoreInput )
      returns (output: Result<Types.WriteNewKeyToStoreOutput, Types.Error>)
      requires
        && ValidState()
        ...
        ==> output.Failure?
    {
      ...
      output := Success(Types.WriteNewKeyToStoreOutput);
    }

    method WriteNewBranchKeyVersionToKeystore' ( input: Types.WriteNewBranchKeyVersionToKeystoreInput )
      returns (output: Result<Types.WriteNewBranchKeyVersionToKeystoreOutput, Types.Error>)
      requires
        && ValidState()
      modifies Modifies - {History}
      // Dafny will skip type parameters when generating a default decreases clause.
                ...
                )
    {
      ...
      var transactWriteItemsResponse? := ddbClient.TransactWriteItems(transactRequest);
      var BRANCH_KEY_ACTIVE_VERSION_FIELD :- transactWriteItemsResponse?
      .MapFailure(e => Types.ComAmazonawsDynamodb(ComAmazonawsDynamodb := e));

      output := Success(Types.WriteNewBranchKeyVersionToKeystoreOutput);
    ...
    method GetTableName' ( input: Types.GetTableNameInput )
      returns (output: Result<Types.GetTableNameOutput, Types.Error>)
      requires
        && ValidState()
      modifies Modifies - {History}
      // Dafny will skip type parameters when generating a default decreases clause.
      ...
      ensures unchanged(History)
    {
      return Success(Types.GetTableNameOutput(
