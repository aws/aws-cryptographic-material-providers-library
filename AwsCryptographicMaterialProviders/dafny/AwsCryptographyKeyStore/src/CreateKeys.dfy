...
module {:options "/functionSyntax:4" } CreateKeys {
  ...
  method CreateBranchAndBeaconKeys(
    ...
    ensures output.Success?
            ==>
              ...
              && |storage.History.WriteNewKeyToStore| == |old(storage.History.WriteNewKeyToStore)| + 1

              && Seq.Last(storage.History.WriteNewKeyToStore).input.Active
              ...
              && Seq.Last(storage.History.WriteNewKeyToStore).input.Version
                 == Structure.ConstructEncryptedHierarchicalKey(
              ...
              && Seq.Last(storage.History.WriteNewKeyToStore).input.Beacon
                 == Structure.ConstructEncryptedHierarchicalKey(
              ...
              && Seq.Last(storage.History.WriteNewKeyToStore).output.Success?
    ...
    ensures
      ...
      || (&& |storage.History.WriteNewKeyToStore| == |old(storage.History.WriteNewKeyToStore)| + 1
          && Seq.Last(storage.History.WriteNewKeyToStore).output.Failure?
          ==> output.Failure?)

  {
    ...
    var _ :- storage.WriteNewKeyToStore(
      Types.WriteNewKeyToStoreInput(
  ...
  method VersionActiveBranchKey(
    ...
    ensures output.Success?
            ==>
              ...
              && |storage.History.WriteNewBranchKeyVersionToKeystore| == |old(storage.History.WriteNewBranchKeyVersionToKeystore)| + 1
              ...
              && Seq.Last(storage.History.WriteNewBranchKeyVersionToKeystore).input.Active
                 == Structure.ConstructEncryptedHierarchicalKey(
                      Seq.Last(kmsClient.History.ReEncrypt).input.DestinationEncryptionContext.value,
                      Seq.Last(kmsClient.History.ReEncrypt).output.value.CiphertextBlob.value
                    )

              && Seq.Last(storage.History.WriteNewBranchKeyVersionToKeystore).input.Version
                 == Structure.ConstructEncryptedHierarchicalKey(
                      Seq.Last(kmsClient.History.GenerateDataKeyWithoutPlaintext).input.EncryptionContext.value,
                      Seq.Last(kmsClient.History.GenerateDataKeyWithoutPlaintext).output.value.CiphertextBlob.value
                    )
              ...
              && Seq.Last(storage.History.WriteNewBranchKeyVersionToKeystore).input.Active.KmsArn == oldActiveItem.KmsArn
              && Seq.Last(storage.History.WriteNewBranchKeyVersionToKeystore).input.Version.KmsArn == oldActiveItem.KmsArn

              && Seq.Last(storage.History.WriteNewBranchKeyVersionToKeystore).output.Success?
    ...
    ensures
      ...
      || (&& |storage.History.GetEncryptedActiveBranchKey| == |old(storage.History.GetEncryptedActiveBranchKey)| + 1
          && Seq.Last(storage.History.WriteNewBranchKeyVersionToKeystore).output.Failure?
          ==> output.Failure?)
...

  {
    ...
    var _ :- storage.WriteNewBranchKeyVersionToKeystore(
      Types.WriteNewBranchKeyVersionToKeystoreInput(
        Active :=  Structure.ConstructEncryptedHierarchicalKey(
          activeEncryptionContext,
          wrappedActiveBranchKey.CiphertextBlob.value
        ),
