  ...
  method {:isolate_assertions} VersionActiveBranchKeyVersion2(
    ...
    ensures output.Success? ==>
              ...
              && HvUtils.IsValidHV2EC?(activeItem.EncryptionContext)
                 //= aws-encryption-sdk-specification/framework/branch-key-store.md#authenticating-a-branch-keystore-item-for-item-with-hierarchy-version-v2
                 //= type=implication
                 //# The [AWS KMS Encryption Context](#aws-kms-encryption-context) MUST be the [encryption context](./structures.md##encryption-context), which is built from the branch key context by extracting the keys with prefix `aws-crypto-ec:` and then dropping the prefix.
