...
module MaterialWrapping {
  ...
  datatype GenerateAndWrapInput = GenerateAndWrapInput(
    ...
    nameonly serializedEC:Result<seq<uint8>, Types.Error>
  )
  ...
  // We explicitly separate `WrapMaterial` and `GenerateAndWrap` as separate Actions because:
  ...
  //   2. By separating these Actions, we can have EdkWrapping implement the logic
  ...
  //      switch on the existence of an input pdk correctly.
  //
    ...
    extends ActionWithResult<GenerateAndWrapInput, GenerateAndWrapOutput<T>, Types.Error>
  {
    ...
    predicate Requires(input: GenerateAndWrapInput)
      : (outcome: bool)
    {
    ...
    extends ActionWithResult<WrapInput, WrapOutput<T>, Types.Error>
  {
    ...
    predicate Requires(input: WrapInput)
      : (outcome: bool)
    {
      && input.serializedEC == CanonicalEncryptionContext.EncryptionContextToAAD(input.encryptionContext)
    }
    ...
    extends ActionWithResult<UnwrapInput, UnwrapOutput<T>, Types.Error>
  {
    ...
    predicate Requires(input: UnwrapInput)
      : (outcome: bool)
    {
      && input.serializedEC == CanonicalEncryptionContext.EncryptionContextToAAD(input.encryptionContext)
    }
