...
module {:options "/functionSyntax:4" } TestDecryptEncryptStrat {
  ...
  method {:test} TestMutateHv2ToHv2ApplyHappyCase()
  {
    // print " running";
    ...
    var testId := "test-mutate-hv-2" + "-" + uuid;

    AdminFixtures.CreateHappyCaseId(id:=testId, strategy := simpleStrategy, hierarchyVersion := KeyStoreTypes.HierarchyVersion.v2, admin? := Some(underTest), customEC := map[UTF8.EncodeAscii("Koda") := UTF8.EncodeAscii("Is a dog.")]);
