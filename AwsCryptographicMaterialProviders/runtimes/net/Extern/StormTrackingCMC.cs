...
namespace software.amazon.cryptography.internaldafny.StormTrackingCMC
{
    ...
    public partial class StormTrackingCMC : DafnyTypes.ICryptographicMaterialsCache
    {
        ...
        {
            int capacity = (int)cmc.wrapped.entryCapacity;
        ...
        private static long NowSeconds() => (long)(DateTime.Now - DateTime.MinValue).TotalSeconds;
            ...
            while (true)
            {
                ...
                switch (state)
                {
                    ...
                    case CacheState.EmptyWait wait:
                        ...
                        // Wait on signal instead of Thread.Sleep polling
        ...
        private CacheState GetFromCache(ByteArrayKey key)
        {
            ...
            if (_cache.TryGet(key, out var entry))
            {
                ...
                // Check if in grace period: expiryTime is in seconds, gracePeriod in millis
        ...
        private CacheState CheckNewEntry(ByteArrayKey key, long now)
        {
            ...
            var entry = new InFlightEntry { Timestamp = now };
            _inFlight[key] = entry;
            return CacheState.EmptyFetch.Instance;
        }
        ...
        {
            // This method exists for interface compatibility but shouldn't be called
            // in the native path. Delegate to the wrapped Dafny StormTracker as fallback.
