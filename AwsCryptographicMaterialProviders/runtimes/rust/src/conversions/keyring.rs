...
>
{
    let inner_input = crate::conversions::on_encrypt::_on_encrypt_input::from_dafny(input.clone());
...
>
{
    let inner_input = crate::conversions::on_decrypt::_on_decrypt_input::from_dafny(input.clone());
...
> {
  ...
  if matches!(
      ...
      Ok(
          crate::conversions::on_encrypt::_on_encrypt_output::from_dafny(inner_result.value().clone()),
      )
  } else {
...
> {
  ...
  if matches!(
      ...
      Ok(
          crate::conversions::on_decrypt::_on_decrypt_output::from_dafny(inner_result.value().clone()),
      )
  } else {
      Err(crate::conversions::error::from_dafny(
          inner_result.error().clone(),
