...
class StormTracker:
    ...
    def InFlightSize(self):
        d_882_x_ = (self.inFlight).Size()
        if (d_882_x_) <= ((StandardLibrary_mUInt.default__).INT64__MAX__LIMIT):
            return d_882_x_
        elif True:
    ...
    def CheckInFlight(self, identifier, result, now):
        ...
        d_883_fanOutReached_: bool
        out180_: bool
        out180_ = (self).FanOutReached(now)
        d_883_fanOutReached_ = out180_
        if d_883_fanOutReached_:
            output = StormTracker.CacheState_Full(result)
            return output
        elif ((result).expiryTime) <= (now):
            out181_: StormTracker.CacheState
            out181_ = (self).CheckNewEntry(identifier, now)
            output = out181_
            ...
            if (self.inFlight).HasKey(identifier):
                d_884_entry_: BoundedInts.int64
                d_884_entry_ = (self.inFlight).Select(identifier)
                if ((self).AddLong(d_884_entry_, self.graceInterval)) > (now):
                    output = StormTracker.CacheState_Full(result)
                    return output
    ...
    def PruneInFlight(self, now):
        ...
        if (self.lastPrune) == (now):
            return
        (self).lastPrune = now
        d_885_keySet_: _dafny.Set
        d_885_keySet_ = (self.inFlight).Keys()
        d_886_keys_: _dafny.Seq
        d_886_keys_ = SortedSets.default__.SetToSequence(d_885_keySet_)
        hi8_: int = len(d_886_keys_)
        for d_887_i_ in range(0, hi8_):
            d_888_v_: BoundedInts.int64
            d_888_v_ = (self.inFlight).Select((d_886_keys_)[d_887_i_])
            if (now) >= ((self).AddLong(d_888_v_, self.inFlightTTL)):
                (self.inFlight).Remove((d_886_keys_)[d_887_i_])

    def CheckNewEntry(self, identifier, now):
        output: StormTracker.CacheState = StormTracker.CacheState_EmptyWait.default()()
        d_889_fanOutReached_: bool
        out182_: bool
        out182_ = (self).FanOutReached(now)
        d_889_fanOutReached_ = out182_
        if d_889_fanOutReached_:
            output = StormTracker.CacheState_EmptyWait()
            return output
        elif (self.inFlight).HasKey(identifier):
            d_890_entry_: BoundedInts.int64
            d_890_entry_ = (self.inFlight).Select(identifier)
            if ((self).AddLong(d_890_entry_, self.graceInterval)) > (now):
                output = StormTracker.CacheState_EmptyWait()
                return output
        (self.inFlight).Put(identifier, now)
        output = StormTracker.CacheState_EmptyFetch()
    ...
    def GetFromCacheWithTime(self, input, now):
        ...
        d_891_result_: Wrappers.Result
        out183_: Wrappers.Result
        out183_ = (self.wrapped).GetCacheEntryWithTime(input, now)
        d_891_result_ = out183_
        if (d_891_result_).is_Success:
            d_892_newResult_: StormTracker.CacheState
            out184_: StormTracker.CacheState
            out184_ = (self).CheckInFlight((input).identifier, (d_891_result_).value, now)
            d_892_newResult_ = out184_
            output = Wrappers.Result_Success(d_892_newResult_)
            return output
        elif ((d_891_result_).error).is_EntryDoesNotExist:
            d_893_newResult_: StormTracker.CacheState
            out185_: StormTracker.CacheState
            out185_ = (self).CheckNewEntry((input).identifier, now)
            d_893_newResult_ = out185_
            output = Wrappers.Result_Success(d_893_newResult_)
            return output
        elif True:
            output = Wrappers.Result_Failure((d_891_result_).error)
            return output
        return output

    def GetFromCache(self, input):
        output: Wrappers.Result = Wrappers.Result_Success.default(StormTracker.CacheState.default())()
        d_894_now_: BoundedInts.int64
        out186_: BoundedInts.int64
        out186_ = Time.default__.CurrentRelativeTime()
        d_894_now_ = out186_
        out187_: Wrappers.Result
        out187_ = (self).GetFromCacheWithTime(input, d_894_now_)
        output = out187_
        return output

    def GetCacheEntry(self, input):
        output: Wrappers.Result = None
        d_895_result_: Wrappers.Result
        out188_: Wrappers.Result
        out188_ = (self).GetFromCache(input)
        d_895_result_ = out188_
        if (d_895_result_).is_Failure:
            output = Wrappers.Result_Failure((d_895_result_).error)
            return output
        elif ((d_895_result_).value).is_Full:
            output = Wrappers.Result_Success(((d_895_result_).value).data)
            return output
        elif True:
    ...
    def PutCacheEntry(self, input):
        ...
        out189_ = (self.wrapped).PutCacheEntry_k(input)
        output = out189_
        return output
    ...
    def DeleteCacheEntry(self, input):
        ...
        out190_ = (self.wrapped).DeleteCacheEntry_k(input)
        output = out190_
        return output
    ...
    def UpdateUsageMetadata(self, input):
        ...
        out191_ = (self.wrapped).UpdateUsageMetadata_k(input)
        output = out191_
        return output
