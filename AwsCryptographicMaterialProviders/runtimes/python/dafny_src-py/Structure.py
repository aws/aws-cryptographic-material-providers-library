...
class default__:
        ...
        def iife2_():
            ...
            for compr_2_ in ((((encryptionContext).keys) | (_dafny.Set({(Structure.default__).BRANCH__KEY__FIELD}))) - (_dafny.Set({(Structure.default__).TABLE__FIELD}))).Elements:
                ...
                if software_amazon_cryptography_services_dynamodb_internaldafny_types.default__.IsValid__AttributeName(d_100_k_):
                    if (d_100_k_) in ((((encryptionContext).keys) | (_dafny.Set({(Structure.default__).BRANCH__KEY__FIELD}))) - (_dafny.Set({(Structure.default__).TABLE__FIELD}))):
                        coll2_[d_100_k_] = (software_amazon_cryptography_services_dynamodb_internaldafny_types.AttributeValue_N((encryptionContext)[(Structure.default__).HIERARCHY__VERSION]) if (d_100_k_) == ((Structure.default__).HIERARCHY__VERSION) else (software_amazon_cryptography_services_dynamodb_internaldafny_types.AttributeValue_B(encryptedKey) if (d_100_k_) == ((Structure.default__).BRANCH__KEY__FIELD) else software_amazon_cryptography_services_dynamodb_internaldafny_types.AttributeValue_S((encryptionContext)[d_100_k_])))
    ...
    def ToBranchKeyContext(item, logicalKeyStoreName):
        def iife3_():
            ...
            for compr_3_ in ((((item).keys) - (_dafny.Set({(Structure.default__).BRANCH__KEY__FIELD}))) | (_dafny.Set({(Structure.default__).TABLE__FIELD}))).Elements:
                ...
                if (d_101_k_) in ((((item).keys) - (_dafny.Set({(Structure.default__).BRANCH__KEY__FIELD}))) | (_dafny.Set({(Structure.default__).TABLE__FIELD}))):
                    coll3_[d_101_k_] = (((item)[d_101_k_]).N if (d_101_k_) == ((Structure.default__).HIERARCHY__VERSION) else (logicalKeyStoreName if (d_101_k_) == ((Structure.default__).TABLE__FIELD) else ((item)[d_101_k_]).S))
    ...
    def ToBranchKeyMaterials(encryptionContext, plaintextKey):
        d_102_versionInformation_ = ((encryptionContext)[(Structure.default__).BRANCH__KEY__ACTIVE__VERSION__FIELD] if ((Structure.default__).BRANCH__KEY__ACTIVE__VERSION__FIELD) in (encryptionContext) else (encryptionContext)[(Structure.default__).TYPE__FIELD])
        d_103_branchKeyVersion_ = _dafny.Seq((d_102_versionInformation_)[len((Structure.default__).BRANCH__KEY__TYPE__PREFIX)::])
        ...
        if (d_104_valueOrError0_).IsFailure():
            ...
            d_107_valueOrError1_ = Structure.default__.ExtractCustomEncryptionContext(encryptionContext)
            if (d_107_valueOrError1_).IsFailure():
                ...
                return Wrappers.Result_Success(software_amazon_cryptography_keystore_internaldafny_types.BranchKeyMaterials_BranchKeyMaterials((encryptionContext)[(Structure.default__).BRANCH__KEY__IDENTIFIER__FIELD], d_106_branchKeyVersionUtf8_, d_108_customEncryptionContext_, plaintextKey))

    @staticmethod
    def ToBeaconKeyMaterials(encryptionContext, plaintextKey):
        d_109_valueOrError0_ = Structure.default__.ExtractCustomEncryptionContext(encryptionContext)
        if (d_109_valueOrError0_).IsFailure():
            ...
            return Wrappers.Result_Success(software_amazon_cryptography_keystore_internaldafny_types.BeaconKeyMaterials_BeaconKeyMaterials((encryptionContext)[(Structure.default__).BRANCH__KEY__IDENTIFIER__FIELD], d_110_customEncryptionContext_, Wrappers.Option_Some(plaintextKey), Wrappers.Option_None()))
    ...
    def ExtractCustomEncryptionContext(encryptionContext):
        def iife4_():
            ...
            for compr_4_ in (encryptionContext).keys.Elements:
                ...
                if ((d_112_k_) in (encryptionContext)) and (((Structure.default__).ENCRYPTION__CONTEXT__PREFIX) < (d_112_k_)):
                    coll4_ = coll4_.union(_dafny.Set([(UTF8.default__.Encode(_dafny.Seq((d_112_k_)[len((Structure.default__).ENCRYPTION__CONTEXT__PREFIX)::])), UTF8.default__.Encode((encryptionContext)[d_112_k_]))]))
        ...
        def iife6_():
            ...
            for compr_6_ in (customEncryptionContext).keys.Elements:
                ...
                if (d_116_k_) in (customEncryptionContext):
                    coll6_[((Structure.default__).ENCRYPTION__CONTEXT__PREFIX) + (d_116_k_)] = (customEncryptionContext)[d_116_k_]
    ...
    def BranchKeyItem_q(m):
        def lambda10_(forall_var_6_):
            ...
            return not ((d_117_k_) in (((m).keys) - (_dafny.Set({(Structure.default__).BRANCH__KEY__FIELD, (Structure.default__).HIERARCHY__VERSION})))) or (((m)[d_117_k_]).is_S)
