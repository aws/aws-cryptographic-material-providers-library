...
class default__:
            ...
            def iife0_():
                ...
                for compr_0_ in ((utf8EncCtx).keys).Elements:
                    ...
                    if (d_65_utf8Key_) in ((utf8EncCtx).keys):
                        coll0_[d_65_utf8Key_] = AwsKmsUtils.default__.StringifyEncryptionContextPair(d_65_utf8Key_, (utf8EncCtx)[d_65_utf8Key_])
    ...
    def StringifyEncryptionContextPair(utf8Key, utf8Value):
        d_71_valueOrError0_ = (UTF8.default__.Decode(utf8Key)).MapFailure(AwsKmsUtils.default__.WrapStringToError)
        if (d_71_valueOrError0_).IsFailure():
            return (d_71_valueOrError0_).PropagateFailure()
        elif True:
            d_72_key_ = (d_71_valueOrError0_).Extract()
            d_73_valueOrError1_ = (UTF8.default__.Decode(utf8Value)).MapFailure(AwsKmsUtils.default__.WrapStringToError)
    ...
    def ValidateKmsKeyId(keyId):
        d_75_valueOrError0_ = (AwsArnParsing.default__.ParseAwsKmsIdentifier(keyId)).MapFailure(AwsKmsUtils.default__.WrapStringToError)
    ...
    def ParseKeyNamespaceAndName(keyNamespace, keyName):
        ...
        if (d_83_valueOrError0_).IsFailure():
            ...
            d_86_valueOrError1_ = Wrappers.default__.Need((len(d_85_namespace_)) < ((StandardLibrary_mUInt.default__).UINT16__LIMIT), software_amazon_cryptography_materialproviders_internaldafny_types.Error_AwsCryptographicMaterialProvidersException(_dafny.Seq("Key namespace too long")))
            if (d_86_valueOrError1_).IsFailure():
                ...
                if (d_87_valueOrError2_).IsFailure():
                    ...
                    d_90_valueOrError3_ = Wrappers.default__.Need((len(d_89_name_)) < ((StandardLibrary_mUInt.default__).UINT16__LIMIT), software_amazon_cryptography_materialproviders_internaldafny_types.Error_AwsCryptographicMaterialProvidersException(_dafny.Seq("Key name too long")))
...
class OnDecryptMrkAwareEncryptedDataKeyFilter(Actions.DeterministicActionWithResult, Actions.DeterministicAction):
    ...
    def Invoke(self, edk):
        ...
        d_96_valueOrError0_: Wrappers.Result = Wrappers.Result_Success.default(_dafny.Seq)()
        d_96_valueOrError0_ = (UTF8.default__.Decode((edk).keyProviderInfo)).MapFailure(AwsKmsUtils.default__.WrapStringToError)
        ...
        d_98_valueOrError1_ = (AwsArnParsing.default__.ParseAwsKmsArn(d_95_keyId_)).MapFailure(AwsKmsUtils.default__.WrapStringToError)
