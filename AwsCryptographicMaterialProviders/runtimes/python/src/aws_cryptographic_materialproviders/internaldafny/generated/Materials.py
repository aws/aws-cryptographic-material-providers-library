...
class default__:
    ...
    def InitializeEncryptionMaterials(input):
        ...
        d_316_valueOrError0_ = Wrappers.default__.Need((default__.EC__PUBLIC__KEY__FIELD) not in ((input).encryptionContext), software_amazon_cryptography_materialproviders_internaldafny_types.Error_AwsCryptographicMaterialProvidersException(_dafny.Seq("Encryption Context ")))
        if (d_316_valueOrError0_).IsFailure():
            ...
            if (d_317_valueOrError1_).IsFailure():
                ...
                d_320_valueOrError2_ = Wrappers.default__.Need((((d_319_suite_).signature).is_ECDSA) == ((((input).signingKey).is_Some) and (((input).verificationKey).is_Some)), software_amazon_cryptography_materialproviders_internaldafny_types.Error_AwsCryptographicMaterialProvidersException(_dafny.Seq("Missing signature key for signed suite.")))
                if (d_320_valueOrError2_).IsFailure():
                    return (d_320_valueOrError2_).PropagateFailure()
                elif True:
                    d_321_valueOrError3_ = Wrappers.default__.Need((((d_319_suite_).signature).is_None) == ((((input).signingKey).is_None) and (((input).verificationKey).is_None)), software_amazon_cryptography_materialproviders_internaldafny_types.Error_AwsCryptographicMaterialProvidersException(_dafny.Seq("Signature key not allowed for non-signed suites.")))
                    if (d_321_valueOrError3_).IsFailure():
                        ...
                        if (d_322_valueOrError4_).IsFailure():
                            ...
                            return Wrappers.Result_Success(software_amazon_cryptography_materialproviders_internaldafny_types.EncryptionMaterials_EncryptionMaterials(d_319_suite_, d_330_encryptionContext_, _dafny.Seq([]), (input).requiredEncryptionContextKeys, Wrappers.Option_None(), (input).signingKey, (Wrappers.Option_None() if ((d_319_suite_).symmetricSignature).is_None else Wrappers.Option_Some(_dafny.Seq([])))))
    ...
    def InitializeDecryptionMaterials(input):
        ...
        if (d_331_valueOrError0_).IsFailure():
            ...
            d_334_valueOrError1_ = Wrappers.default__.Need((((d_333_suite_).signature).is_ECDSA) == ((default__.EC__PUBLIC__KEY__FIELD) in ((input).encryptionContext)), software_amazon_cryptography_materialproviders_internaldafny_types.Error_AwsCryptographicMaterialProvidersException(_dafny.Seq("Encryption Context missing verification key.")))
            if (d_334_valueOrError1_).IsFailure():
                return (d_334_valueOrError1_).PropagateFailure()
            elif True:
                d_335_valueOrError2_ = Wrappers.default__.Need((((d_333_suite_).signature).is_None) == ((default__.EC__PUBLIC__KEY__FIELD) not in ((input).encryptionContext)), software_amazon_cryptography_materialproviders_internaldafny_types.Error_AwsCryptographicMaterialProvidersException(_dafny.Seq("Verification key can not exist in non-signed Algorithm Suites.")))
                if (d_335_valueOrError2_).IsFailure():
                    ...
                    if (d_336_valueOrError3_).IsFailure():
                        ...
                        return Wrappers.Result_Success(software_amazon_cryptography_materialproviders_internaldafny_types.DecryptionMaterials_DecryptionMaterials(d_333_suite_, (input).encryptionContext, (input).requiredEncryptionContextKeys, Wrappers.Option_None(), d_337_verificationKey_, Wrappers.Option_None()))
    ...
    def DecodeVerificationKey(encryptionContext):
        if (default__.EC__PUBLIC__KEY__FIELD) in (encryptionContext):
            ...
            def lambda35_(d_340_e_):
                return software_amazon_cryptography_materialproviders_internaldafny_types.Error_AwsCryptographicMaterialProvidersException(d_340_e_)
            ...
            if (d_339_valueOrError0_).IsFailure():
                ...
                def lambda36_(d_343_e_):
                    return software_amazon_cryptography_materialproviders_internaldafny_types.Error_AwsCryptographicMaterialProvidersException(d_343_e_)
    ...
    def EncryptionMaterialAddEncryptedDataKeys(encryptionMaterials, encryptedDataKeysToAdd, symmetricSigningKeysToAdd):
        d_347_valueOrError0_ = Wrappers.default__.Need(default__.ValidEncryptionMaterials(encryptionMaterials), software_amazon_cryptography_materialproviders_internaldafny_types.Error_InvalidEncryptionMaterialsTransition(_dafny.Seq("Attempt to modify invalid encryption material.")))
        if (d_347_valueOrError0_).IsFailure():
            return (d_347_valueOrError0_).PropagateFailure()
        elif True:
            d_348_valueOrError1_ = Wrappers.default__.Need(((encryptionMaterials).plaintextDataKey).is_Some, software_amazon_cryptography_materialproviders_internaldafny_types.Error_InvalidEncryptionMaterialsTransition(_dafny.Seq("Adding encrypted data keys without a plaintext data key is not allowed.")))
    ...
    def EncryptionMaterialAddDataKey(encryptionMaterials, plaintextDataKey, encryptedDataKeysToAdd, symmetricSigningKeysToAdd):
        ...
        d_353_valueOrError0_ = Wrappers.default__.Need(default__.ValidEncryptionMaterials(encryptionMaterials), software_amazon_cryptography_materialproviders_internaldafny_types.Error_InvalidEncryptionMaterialsTransition(_dafny.Seq("Attempt to modify invalid encryption material.")))
        if (d_353_valueOrError0_).IsFailure():
            return (d_353_valueOrError0_).PropagateFailure()
        elif True:
            d_354_valueOrError1_ = Wrappers.default__.Need(((encryptionMaterials).plaintextDataKey).is_None, software_amazon_cryptography_materialproviders_internaldafny_types.Error_InvalidEncryptionMaterialsTransition(_dafny.Seq("Attempt to modify plaintextDataKey.")))
            if (d_354_valueOrError1_).IsFailure():
                return (d_354_valueOrError1_).PropagateFailure()
            elif True:
                d_355_valueOrError2_ = Wrappers.default__.Need((AlgorithmSuites.default__.GetEncryptKeyLength(d_352_suite_)) == (len(plaintextDataKey)), software_amazon_cryptography_materialproviders_internaldafny_types.Error_InvalidEncryptionMaterialsTransition(_dafny.Seq("plaintextDataKey does not match Algorithm Suite specification.")))
    ...
    def DecryptionMaterialsAddDataKey(decryptionMaterials, plaintextDataKey, symmetricSigningKey):
        ...
        d_362_valueOrError0_ = Wrappers.default__.Need(default__.ValidDecryptionMaterials(decryptionMaterials), software_amazon_cryptography_materialproviders_internaldafny_types.Error_InvalidDecryptionMaterialsTransition(_dafny.Seq("Attempt to modify invalid decryption material.")))
        if (d_362_valueOrError0_).IsFailure():
            return (d_362_valueOrError0_).PropagateFailure()
        elif True:
            d_363_valueOrError1_ = Wrappers.default__.Need(((decryptionMaterials).plaintextDataKey).is_None, software_amazon_cryptography_materialproviders_internaldafny_types.Error_InvalidDecryptionMaterialsTransition(_dafny.Seq("Attempt to modify plaintextDataKey.")))
            if (d_363_valueOrError1_).IsFailure():
                return (d_363_valueOrError1_).PropagateFailure()
            elif True:
                d_364_valueOrError2_ = Wrappers.default__.Need((AlgorithmSuites.default__.GetEncryptKeyLength(d_361_suite_)) == (len(plaintextDataKey)), software_amazon_cryptography_materialproviders_internaldafny_types.Error_InvalidDecryptionMaterialsTransition(_dafny.Seq("plaintextDataKey does not match Algorithm Suite specification.")))
...
class DecryptionMaterialsPendingPlaintextDataKey:
    ...
    def _Is(source__):
        d_368_d_: software_amazon_cryptography_materialproviders_internaldafny_types.DecryptionMaterials = source__
...
class SealedDecryptionMaterials:
    ...
    def default():
        return software_amazon_cryptography_materialproviders_internaldafny_types.DecryptionMaterials.default()()
    def _Is(source__):
        d_369_d_: software_amazon_cryptography_materialproviders_internaldafny_types.DecryptionMaterials = source__
