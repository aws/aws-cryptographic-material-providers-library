...
class default__:
    ...
    def GetKeyStoreInfo(config):
        ...
        output = Wrappers.Result_Success(software.amazon.cryptography.keystore.internaldafny.types.GetKeyStoreInfoOutput_GetKeyStoreInfoOutput((config).id, (config).ddbTableName, (config).logicalKeyStoreName, (config).grantTokens, (config).kmsConfiguration))
        return output
    ...
    def CreateKeyStore(config, input):
        ...
        d_237_valueOrError1_ = Wrappers.default__.Need(((d_236_tableName_).is_Success) and (((d_236_tableName_).value) == ((config).ddbTableName)), software.amazon.cryptography.keystore.internaldafny.types.Error_KeyStoreException(_dafny.Seq("Configured DDB Table Name does not match parsed Table Name from DDB Table Arn.")))
        if (d_237_valueOrError1_).IsFailure():
            output = (d_237_valueOrError1_).PropagateFailure()
            return output
        output = Wrappers.Result_Success(software.amazon.cryptography.keystore.internaldafny.types.CreateKeyStoreOutput_CreateKeyStoreOutput(d_234_ddbTableArn_))
        return output
    ...
    def CreateKey(config, input):
        ...
        d_238_valueOrError0_ = Wrappers.default__.Need(not (((input).branchKeyIdentifier).is_Some) or ((((input).encryptionContext).is_Some) and ((0) < (len(((input).encryptionContext).value)))), software.amazon.cryptography.keystore.internaldafny.types.Error_KeyStoreException(_dafny.Seq("Custom branch key id requires custom encryption context.")))
        ...
        if ((input).branchKeyIdentifier).is_None:
            ...
            def lambda20_(d_242_e_):
                return software.amazon.cryptography.keystore.internaldafny.types.Error_KeyStoreException(d_242_e_)
            ...
            d_243_valueOrError2_ = Wrappers.default__.Need((0) < (len(((input).branchKeyIdentifier).value)), software.amazon.cryptography.keystore.internaldafny.types.Error_KeyStoreException(_dafny.Seq("Custom branch key id can not be an empty string.")))
        ...
        def lambda21_(d_246_e_):
            return software.amazon.cryptography.keystore.internaldafny.types.Error_KeyStoreException(d_246_e_)
        ...
        def lambda22_(d_250_e_):
            return software.amazon.cryptography.keystore.internaldafny.types.Error_KeyStoreException(d_250_e_)
        ...
        def lambda23_(forall_var_7_):
            ...
            return not ((d_255_i_) in (d_252_encodedEncryptionContext_)) or ((((((d_255_i_)[0]).is_Success) and (((d_255_i_)[1]).is_Success)) and (software.amazon.cryptography.services.dynamodb.internaldafny.types.default__.IsValid__AttributeName((Structure.default__.ENCRYPTION__CONTEXT__PREFIX) + (((d_255_i_)[0]).value)))) and (iife8_(UTF8.default__.Encode(((d_255_i_)[0]).value))))

        d_254_valueOrError5_ = Wrappers.default__.Need(_dafny.quantifier((d_252_encodedEncryptionContext_).Elements, True, lambda23_), software.amazon.cryptography.keystore.internaldafny.types.Error_KeyStoreException(_dafny.Seq("Unable to encode string")))
    ...
    def VersionKey(config, input):
        ...
        d_258_valueOrError0_ = Wrappers.default__.Need((0) < (len((input).branchKeyIdentifier)), software.amazon.cryptography.keystore.internaldafny.types.Error_KeyStoreException(_dafny.Seq("Empty string not supported for identifier.")))
        ...
        def lambda24_(d_261_e_):
            return software.amazon.cryptography.keystore.internaldafny.types.Error_KeyStoreException(d_261_e_)
        ...
        def lambda25_(d_265_e_):
            return software.amazon.cryptography.keystore.internaldafny.types.Error_KeyStoreException(d_265_e_)
...
class Config:
    @classmethod
    def default(cls, ):
        return lambda: Config_Config(_dafny.Seq({}), _dafny.Seq({}), _dafny.Seq({}), software.amazon.cryptography.keystore.internaldafny.types.KMSConfiguration.default()(), _dafny.Seq({}), None, None)
