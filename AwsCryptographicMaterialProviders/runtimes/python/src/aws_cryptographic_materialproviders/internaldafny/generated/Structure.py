...
class default__:
    ...
    def BranchKeyContext_q(m):
        def lambda7_(forall_var_4_):
            ...
            return not ((d_109_k_) in ((m).keys)) or (software_amazon_cryptography_services_dynamodb_internaldafny_types.default__.IsValid__AttributeName(d_109_k_))
    ...
    def ToAttributeMap(encryptionContext, encryptedKey):
        def iife2_():
            ...
            for compr_2_ in ((((encryptionContext).keys) | (_dafny.Set({default__.BRANCH__KEY__FIELD}))) - (_dafny.Set({default__.TABLE__FIELD}))).Elements:
                ...
                if software_amazon_cryptography_services_dynamodb_internaldafny_types.AttributeName._Is(d_110_k_):
                    if (d_110_k_) in ((((encryptionContext).keys) | (_dafny.Set({default__.BRANCH__KEY__FIELD}))) - (_dafny.Set({default__.TABLE__FIELD}))):
                        coll2_[d_110_k_] = (software_amazon_cryptography_services_dynamodb_internaldafny_types.AttributeValue_N((encryptionContext)[default__.HIERARCHY__VERSION]) if (d_110_k_) == (default__.HIERARCHY__VERSION) else (software_amazon_cryptography_services_dynamodb_internaldafny_types.AttributeValue_B(encryptedKey) if (d_110_k_) == (default__.BRANCH__KEY__FIELD) else software_amazon_cryptography_services_dynamodb_internaldafny_types.AttributeValue_S((encryptionContext)[d_110_k_])))
    ...
    def ToBranchKeyMaterials(encryptionContext, plaintextKey):
        ...
        def lambda8_(d_115_e_):
            return software_amazon_cryptography_keystore_internaldafny_types.Error_KeyStoreException(d_115_e_)
        ...
        if (d_114_valueOrError0_).IsFailure():
            ...
            if (d_117_valueOrError1_).IsFailure():
                ...
                return Wrappers.Result_Success(software_amazon_cryptography_keystore_internaldafny_types.BranchKeyMaterials_BranchKeyMaterials((encryptionContext)[default__.BRANCH__KEY__IDENTIFIER__FIELD], d_116_branchKeyVersionUtf8_, d_118_customEncryptionContext_, plaintextKey))
    ...
    def ToBeaconKeyMaterials(encryptionContext, plaintextKey):
        ...
        if (d_119_valueOrError0_).IsFailure():
            ...
            return Wrappers.Result_Success(software_amazon_cryptography_keystore_internaldafny_types.BeaconKeyMaterials_BeaconKeyMaterials((encryptionContext)[default__.BRANCH__KEY__IDENTIFIER__FIELD], d_120_customEncryptionContext_, Wrappers.Option_Some(plaintextKey), Wrappers.Option_None()))
    ...
    def ExtractCustomEncryptionContext(encryptionContext):
        ...
        d_123_valueOrError0_ = Wrappers.default__.Need(_dafny.quantifier((d_121_encodedEncryptionContext_).Elements, True, lambda9_), software_amazon_cryptography_keystore_internaldafny_types.Error_KeyStoreException(_dafny.Seq("Unable to encode string")))
