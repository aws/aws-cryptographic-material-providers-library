...
class default__:
    ...
    def BranchKeyContext_q(m):
        def lambda8_(forall_var_4_):
            ...
            return not ((d_100_k_) in ((m).keys)) or (ComAmazonawsDynamodbTypes.default__.IsValid__AttributeName(d_100_k_))
    ...
    def ToAttributeMap(encryptionContext, encryptedKey):
        def iife4_():
            ...
            for compr_2_ in ((((encryptionContext).keys) | (_dafny.Set({default__.BRANCH__KEY__FIELD}))) - (_dafny.Set({default__.TABLE__FIELD}))).Elements:
                ...
                if ComAmazonawsDynamodbTypes.AttributeName._Is(d_101_k_):
                    if (d_101_k_) in ((((encryptionContext).keys) | (_dafny.Set({default__.BRANCH__KEY__FIELD}))) - (_dafny.Set({default__.TABLE__FIELD}))):
                        coll2_[d_101_k_] = (ComAmazonawsDynamodbTypes.AttributeValue_N((encryptionContext)[default__.HIERARCHY__VERSION]) if (d_101_k_) == (default__.HIERARCHY__VERSION) else (ComAmazonawsDynamodbTypes.AttributeValue_B(encryptedKey) if (d_101_k_) == (default__.BRANCH__KEY__FIELD) else ComAmazonawsDynamodbTypes.AttributeValue_S((encryptionContext)[d_101_k_])))
    ...
    def ToBranchKeyContext(item, logicalKeyStoreName):
        def iife5_():
            ...
            for compr_3_ in ((((item).keys) - (_dafny.Set({default__.BRANCH__KEY__FIELD}))) | (_dafny.Set({default__.TABLE__FIELD}))).Elements:
                ...
                if (d_102_k_) in ((((item).keys) - (_dafny.Set({default__.BRANCH__KEY__FIELD}))) | (_dafny.Set({default__.TABLE__FIELD}))):
                    coll3_[d_102_k_] = (((item)[d_102_k_]).N if (d_102_k_) == (default__.HIERARCHY__VERSION) else (logicalKeyStoreName if (d_102_k_) == (default__.TABLE__FIELD) else ((item)[d_102_k_]).S))
    ...
    def ToBranchKeyMaterials(encryptionContext, plaintextKey):
        d_103_versionInformation_ = ((encryptionContext)[default__.BRANCH__KEY__ACTIVE__VERSION__FIELD] if (default__.BRANCH__KEY__ACTIVE__VERSION__FIELD) in (encryptionContext) else (encryptionContext)[default__.TYPE__FIELD])
        d_104_branchKeyVersion_ = _dafny.Seq((d_103_versionInformation_)[len(default__.BRANCH__KEY__TYPE__PREFIX)::])
        def lambda9_(d_106_e_):
            return AwsCryptographyKeyStoreTypes.Error_KeyStoreException(d_106_e_)

        d_105_valueOrError0_ = (UTF8.default__.Encode(d_104_branchKeyVersion_)).MapFailure(lambda9_)
        if (d_105_valueOrError0_).IsFailure():
            ...
            d_108_valueOrError1_ = default__.ExtractCustomEncryptionContext(encryptionContext)
            if (d_108_valueOrError1_).IsFailure():
                ...
                return Wrappers.Result_Success(AwsCryptographyKeyStoreTypes.BranchKeyMaterials_BranchKeyMaterials((encryptionContext)[default__.BRANCH__KEY__IDENTIFIER__FIELD], d_107_branchKeyVersionUtf8_, d_109_customEncryptionContext_, plaintextKey))

    @staticmethod
    def ToBeaconKeyMaterials(encryptionContext, plaintextKey):
        d_110_valueOrError0_ = default__.ExtractCustomEncryptionContext(encryptionContext)
        if (d_110_valueOrError0_).IsFailure():
            ...
            return Wrappers.Result_Success(AwsCryptographyKeyStoreTypes.BeaconKeyMaterials_BeaconKeyMaterials((encryptionContext)[default__.BRANCH__KEY__IDENTIFIER__FIELD], d_111_customEncryptionContext_, Wrappers.Option_Some(plaintextKey), Wrappers.Option_None()))
    ...
    def ExtractCustomEncryptionContext(encryptionContext):
        def iife6_():
            ...
            for compr_4_ in (encryptionContext).keys.Elements:
                ...
                if ((d_113_k_) in (encryptionContext)) and ((default__.ENCRYPTION__CONTEXT__PREFIX) < (d_113_k_)):
                    coll4_ = coll4_.union(_dafny.Set([(UTF8.default__.Encode(_dafny.Seq((d_113_k_)[len(default__.ENCRYPTION__CONTEXT__PREFIX)::])), UTF8.default__.Encode((encryptionContext)[d_113_k_]))]))
            return _dafny.Set(coll4_)
        d_112_encodedEncryptionContext_ = iife6_()
        ...
        d_114_valueOrError0_ = Wrappers.default__.Need(_dafny.quantifier((d_112_encodedEncryptionContext_).Elements, True, lambda10_), AwsCryptographyKeyStoreTypes.Error_KeyStoreException(_dafny.Seq("Unable to encode string")))
        ...
        def iife8_():
            ...
            for compr_6_ in (customEncryptionContext).keys.Elements:
                ...
                if (d_117_k_) in (customEncryptionContext):
                    coll6_[(default__.ENCRYPTION__CONTEXT__PREFIX) + (d_117_k_)] = (customEncryptionContext)[d_117_k_]
    ...
    def BranchKeyItem_q(m):
        def lambda11_(forall_var_6_):
            ...
            return not ((d_118_k_) in (((m).keys) - (_dafny.Set({default__.BRANCH__KEY__FIELD, default__.HIERARCHY__VERSION})))) or (((m)[d_118_k_]).is_S)
...
class BranchKeyContext:
    ...
    def default():
        return _dafny.Map({})
    def _Is(source__):
        d_119_m_: _dafny.Map = source__
        return default__.BranchKeyContext_q(d_119_m_)
...
class BranchKeyItem:
    ...
    def default():
        return _dafny.Map({})
    def _Is(source__):
        d_120_m_: _dafny.Map = source__
        return default__.BranchKeyItem_q(d_120_m_)
...
class ActiveBranchKeyItem:
    ...
    def default():
        return _dafny.Map({})
    def _Is(source__):
        d_121_m_: _dafny.Map = source__
        return default__.ActiveBranchKeyItem_q(d_121_m_)
...
class VersionBranchKeyItem:
    ...
    def _Is(source__):
        ...
        return default__.VersionBranchKeyItem_q(d_122_m_)
...
class BeaconKeyItem:
    ...
    def _Is(source__):
        ...
        return default__.BeaconKeyItem_q(d_123_m_)
