...
class default__:
    ...
    def ParseKeyNamespaceAndName(keyNamespace, keyName):
        def lambda4_(d_80_e_):
            return AwsCryptographyMaterialProvidersTypes.Error_AwsCryptographicMaterialProvidersException((_dafny.Seq("Key namespace could not be UTF8-encoded")) + (d_80_e_))

        d_79_valueOrError0_ = (UTF8.default__.Encode(keyNamespace)).MapFailure(lambda4_)
        if (d_79_valueOrError0_).IsFailure():
            return (d_79_valueOrError0_).PropagateFailure()
        elif True:
            d_81_namespace_ = (d_79_valueOrError0_).Extract()
            d_82_valueOrError1_ = Wrappers.default__.Need((len(d_81_namespace_)) < (StandardLibrary_UInt.default__.UINT16__LIMIT), AwsCryptographyMaterialProvidersTypes.Error_AwsCryptographicMaterialProvidersException(_dafny.Seq("Key namespace too long")))
            if (d_82_valueOrError1_).IsFailure():
                return (d_82_valueOrError1_).PropagateFailure()
            elif True:
                def lambda5_(d_84_e_):
                    return AwsCryptographyMaterialProvidersTypes.Error_AwsCryptographicMaterialProvidersException((_dafny.Seq("Key name could not be UTF8-encoded")) + (d_84_e_))

                d_83_valueOrError2_ = (UTF8.default__.Encode(keyName)).MapFailure(lambda5_)
                if (d_83_valueOrError2_).IsFailure():
                    return (d_83_valueOrError2_).PropagateFailure()
                elif True:
                    d_85_name_ = (d_83_valueOrError2_).Extract()
                    d_86_valueOrError3_ = Wrappers.default__.Need((len(d_85_name_)) < (StandardLibrary_UInt.default__.UINT16__LIMIT), AwsCryptographyMaterialProvidersTypes.Error_AwsCryptographicMaterialProvidersException(_dafny.Seq("Key name too long")))
                    if (d_86_valueOrError3_).IsFailure():
                        return (d_86_valueOrError3_).PropagateFailure()
                    elif True:
                        return Wrappers.Result_Success((d_81_namespace_, d_85_name_))

    @staticmethod
    def ValidateDiscoveryFilter(filter):
        d_87_valueOrError0_ = Wrappers.default__.Need((len((filter).accountIds)) > (0), AwsCryptographyMaterialProvidersTypes.Error_AwsCryptographicMaterialProvidersException(_dafny.Seq("Discovery filter must have at least one account ID")))
        if (d_87_valueOrError0_).IsFailure():
            return (d_87_valueOrError0_).PropagateFailure()
        elif True:
            def lambda6_(forall_var_3_):
                d_89_accountId_: _dafny.Seq = forall_var_3_
                return not ((d_89_accountId_) in ((filter).accountIds)) or ((len(d_89_accountId_)) > (0))

            d_88_valueOrError1_ = Wrappers.default__.Need(_dafny.quantifier(((filter).accountIds).UniqueElements, True, lambda6_), AwsCryptographyMaterialProvidersTypes.Error_AwsCryptographicMaterialProvidersException(_dafny.Seq("Discovery filter account IDs cannot be blank")))
            if (d_88_valueOrError1_).IsFailure():
                return (d_88_valueOrError1_).PropagateFailure()
            elif True:
                d_90_valueOrError2_ = Wrappers.default__.Need((len((filter).partition)) > (0), AwsCryptographyMaterialProvidersTypes.Error_AwsCryptographicMaterialProvidersException(_dafny.Seq("Discovery filter partition cannot be blank")))
                if (d_90_valueOrError2_).IsFailure():
                    return (d_90_valueOrError2_).PropagateFailure()
                elif True:
                    return Wrappers.Result_Success(())
...
class OnDecryptMrkAwareEncryptedDataKeyFilter(Actions.DeterministicActionWithResult, Actions.DeterministicAction):
    ...
    def Invoke(self, edk):
        ...
        if not(UTF8.default__.ValidUTF8Seq((edk).keyProviderInfo)):
            ...
            return res
        d_91_valueOrError0_: Wrappers.Result = Wrappers.Result.default(_dafny.Seq)()
        d_91_valueOrError0_ = (UTF8.default__.Decode((edk).keyProviderInfo)).MapFailure(default__.WrapStringToError)
        if (d_91_valueOrError0_).IsFailure():
            res = (d_91_valueOrError0_).PropagateFailure()
            return res
        d_92_keyId_: _dafny.Seq
        d_92_keyId_ = (d_91_valueOrError0_).Extract()
        d_93_valueOrError1_: Wrappers.Result = None
        d_93_valueOrError1_ = (AwsArnParsing.default__.ParseAwsKmsArn(d_92_keyId_)).MapFailure(default__.WrapStringToError)
        if (d_93_valueOrError1_).IsFailure():
            res = (d_93_valueOrError1_).PropagateFailure()
            return res
        d_94_arn_: AwsArnParsing.AwsArn
        d_94_arn_ = (d_93_valueOrError1_).Extract()
        res = Wrappers.Result_Success(AwsKmsMrkMatchForDecrypt.default__.AwsKmsMrkMatchForDecrypt((self).awsKmsKey, AwsArnParsing.AwsKmsIdentifier_AwsKmsArnIdentifier(d_94_arn_)))
        return res
        return res
