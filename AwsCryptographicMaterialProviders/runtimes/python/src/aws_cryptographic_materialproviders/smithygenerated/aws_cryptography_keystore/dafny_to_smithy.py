...
def aws_cryptography_keystore_KMSConfiguration(dafny_input):
    ...
    if isinstance(dafny_input, KMSConfiguration_kmsKeyArn):
        KMSConfiguration_union_value = aws_cryptographic_materialproviders.smithygenerated.aws_cryptography_keystore.models.KMSConfigurationKmsKeyArn(b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.kmsKeyArn).decode('utf-16-be'))
    elif isinstance(dafny_input, KMSConfiguration_kmsMRKeyArn):
        KMSConfiguration_union_value = aws_cryptographic_materialproviders.smithygenerated.aws_cryptography_keystore.models.KMSConfigurationKmsMRKeyArn(b''.join(ord(c).to_bytes(2, 'big') for c in dafny_input.kmsMRKeyArn).decode('utf-16-be'))
