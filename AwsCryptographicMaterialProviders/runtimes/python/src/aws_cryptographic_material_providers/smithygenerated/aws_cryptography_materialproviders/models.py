...
class CreateCachingCMMInput:
    underlying_cmc: 'aws_cryptographic_material_providers.smithygenerated.aws_cryptography_materialproviders.references.CryptographicMaterialsCache'
    cache_limit_ttl_seconds: int
    ...
    def __init__(
        ...
        *,
        ...
        :param underlying_cmc: The Cryptographic Materials Cache the Caching
        Cryptographic Materials Manager will use to cache requests.
        :param cache_limit_ttl_seconds: Sets the maximum lifetime for entries in the
        cache, for both encrypt and decrypt operations. When the specified amount of
        time passes after initial creation of the entry, the entry will be considered
        unusable, and the next operation will incur a cache miss.
        :param underlying_cmm: The Cryptographic Materials Manager that the created
        Caching Cryptographic Materials Manager will delegate to. Either a Keyring or
        underlying Cryptographic Materials Manager must be specified.
        :param keyring: The Keyring that the created Cryptographic Materials Manager
        will use to wrap data keys. The created Caching CMM will delegate to a Default
        Cryptographic Materials Manager created with this Keyring. Either a Keyring or
        an underlying Cryptographic Materials Manager must be specified as input.
        :param partition_key: Sets the partition ID for this CMM. By default, two CMMs
        will never use each other's cache entries. This helps ensure that CMMs with
        different delegates won't incorrectly use each other's encrypt and decrypt
        results. However, in certain special circumstances it can be useful to share
        entries between different CMMs - for example, if the backing CMM is constructed
        based on some parameters that depend on the operation, you may wish for
        delegates constructed with the same parameters to share the same partition. To
        accomplish this, set the same partition ID and backing cache on both CMMs;
        entries cached from one of these CMMs can then be used by the other. This should
        only be done with careful consideration and verification that the CMM delegates
        are equivalent for your application's purposes. By default, the partition ID is
        set to a random UUID to avoid any collisions.
        :param limit_bytes: Sets the maximum number of plaintext bytes that can be
        encrypted under the same cached data key. This does not affect decrypt
        operations. Specifying this limit is optional; by default, the limit is set to
        2^63 - 1. If this limit is set to zero, keys can only be cached if they are used
        for zero-length messages.
        :param limit_messages: Sets the maximum number of individual messages that can
        be encrypted under the same cached data key. This does not affect decrypt
        operations. Specifying this limit is optional; by default, the limit is set to
        2^32. This is also the maximum accepted value.
        """
        self.underlying_cmc = underlying_cmc
        if (cache_limit_ttl_seconds is not None) and (cache_limit_ttl_seconds < 0):
            raise ValueError("cache_limit_ttl_seconds must be greater than or equal to 0")

        self.cache_limit_ttl_seconds = cache_limit_ttl_seconds
    ...
    def as_dict(self) -> Dict[str, Any]:
        """Converts the CreateCachingCMMInput to a dictionary.

        """
        d: Dict[str, Any] = {
            "underlying_cmc": self.underlying_cmc.as_dict(),
        }

        if self.cache_limit_ttl_seconds is not None:
            d["cache_limit_ttl_seconds"] = self.cache_limit_ttl_seconds
        ...
        if self.limit_bytes is not None:
            d["limit_bytes"] = self.limit_bytes

        if self.limit_messages is not None:
            d["limit_messages"] = self.limit_messages

        return d

    @staticmethod
    def from_dict(d: Dict[str, Any]) -> "CreateCachingCMMInput":
        """Creates a CreateCachingCMMInput from a dictionary.
        ...
        kwargs: Dict[str, Any] = {
            "underlying_cmc": CryptographicMaterialsCache.from_dict(d["underlying_cmc"]),
        }

        if "cache_limit_ttl_seconds" in d:
            kwargs["cache_limit_ttl_seconds"] = d["cache_limit_ttl_seconds"]
        ...
        if "partition_key" in d:
            kwargs["partition_key"] = d["partition_key"]

        if "limit_bytes" in d:
            kwargs["limit_bytes"] = d["limit_bytes"]

        if "limit_messages" in d:
            kwargs["limit_messages"] = d["limit_messages"]

        return CreateCachingCMMInput(**kwargs)

    def __repr__(self) -> str:
        result = "CreateCachingCMMInput("
        if self.underlying_cmc is not None:
            result += f"underlying_cmc={repr(self.underlying_cmc)}, "

        if self.cache_limit_ttl_seconds is not None:
            result += f"cache_limit_ttl_seconds={repr(self.cache_limit_ttl_seconds)}, "
        ...
        if self.partition_key is not None:
            result += f"partition_key={repr(self.partition_key)}, "

        if self.limit_bytes is not None:
            result += f"limit_bytes={repr(self.limit_bytes)}, "

        if self.limit_messages is not None:
            result += f"limit_messages={repr(self.limit_messages)}"

        return result + ")"

    def __eq__(self, other: Any) -> bool:
        if not isinstance(other, CreateCachingCMMInput):
            return False
        attributes: list[str] = ['underlying_cmc','cache_limit_ttl_seconds','underlying_cmm','keyring','partition_key','limit_bytes','limit_messages',]
        return all(
            getattr(self, a) == getattr(other, a)
            for a in attributes
        )
