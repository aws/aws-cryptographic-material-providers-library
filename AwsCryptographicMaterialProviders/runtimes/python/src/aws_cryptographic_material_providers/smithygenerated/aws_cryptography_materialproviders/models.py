...
class CreateAwsKmsHierarchicalKeyringInput:
    ...
    def __init__(
        ...
        *,
        ...
        :param cache_warm_up_versions: If set, pre-populates the cache during
        Hierarchical Keyring construction by querying DynamoDB for the active branch key
        and the N most recent branch key versions, then decrypting each via KMS. This
        eliminates cold start latency on the first encrypt or decrypt calls. Only
        supported when branchKeyId is set (not branchKeyIdSupplier). Clamped to a
        maximum of 10 to prevent excessive KMS calls. Defaults to 0 (no warmup).
        """
        ...
        if (cache_warm_up_versions is not None) and (cache_warm_up_versions < 0):
            raise ValueError("cache_warm_up_versions must be greater than or equal to 0")

        self.cache_warm_up_versions = cache_warm_up_versions
    ...
    def as_dict(self) -> Dict[str, Any]:
        ...
        if self.cache_warm_up_versions is not None:
            d["cache_warm_up_versions"] = self.cache_warm_up_versions
    ...
    def from_dict(d: Dict[str, Any]) -> "CreateAwsKmsHierarchicalKeyringInput":
        ...
        if "cache_warm_up_versions" in d:
            kwargs["cache_warm_up_versions"] = d["cache_warm_up_versions"]
    ...
    def __repr__(self) -> str:
        ...
        if self.cache_warm_up_versions is not None:
            result += f"cache_warm_up_versions={repr(self.cache_warm_up_versions)}"
    ...
    def __eq__(self, other: Any) -> bool:
        if not isinstance(other, CreateAwsKmsHierarchicalKeyringInput):
            return False
        attributes: list[str] = ['branch_key_id','branch_key_id_supplier','key_store','ttl_seconds','cache','partition_id','cache_warm_up_versions',]
        return all(
            getattr(self, a) == getattr(other, a)
            for a in attributes
        )
