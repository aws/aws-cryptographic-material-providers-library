...
class KmsMRKey:
    ...
    def __init__(
        ...
        *,
        ...
        """If an MRK ARN is provided, then the ARNs region will be replaced
        with the provided region.

        :param key_arn: This MUST be a KMS MRK ARN; aliases and Single
            Region keys are rejected.
        :param region: MRK ARN will have its region replaced with this.
        """
        if (key_arn is not None) and (len(key_arn) < 1):
            raise ValueError("The size of key_arn must be greater than or equal to 1")

        if (key_arn is not None) and (len(key_arn) > 2048):
            raise ValueError("The size of key_arn must be less than or equal to 2048")
    ...
    def as_dict(self) -> Dict[str, Any]:
        """Converts the KmsMRKey to a dictionary."""
        return {
    ...
    @staticmethod
    def from_dict(d: Dict[str, Any]) -> "KmsMRKey":
        """Creates a KmsMRKey from a dictionary."""
        kwargs: Dict[str, Any] = {
            "key_arn": d["key_arn"],
            "region": d["region"],
        }

        return KmsMRKey(**kwargs)

    def __repr__(self) -> str:
        result = "KmsMRKey("
        if self.key_arn is not None:
            result += f"key_arn={repr(self.key_arn)}, "
    ...
    def __eq__(self, other: Any) -> bool:
        if not isinstance(other, KmsMRKey):
            return False
        attributes: list[str] = [
            "key_arn",
            "region",
...
class KmsSymmetricKeyArnKmsMRKey:
    """If an MRK ARN is provided, then the ARNs region will be replaced with
    the provided region."""

    def __init__(self, value: KmsMRKey):
        self.value = value

    def as_dict(self) -> Dict[str, Any]:
        return {"KmsMRKey": self.value.as_dict()}

    @staticmethod
    def from_dict(d: Dict[str, Any]) -> "KmsSymmetricKeyArnKmsMRKey":
        if len(d) != 1:
            raise TypeError(f"Unions may have exactly 1 value, but found {len(d)}")

        return KmsSymmetricKeyArnKmsMRKey(KmsMRKey.from_dict(d["KmsMRKey"]))

    def __repr__(self) -> str:
        return f"KmsSymmetricKeyArnKmsMRKey(value=repr(self.value))"

    def __eq__(self, other: Any) -> bool:
        if not isinstance(other, KmsSymmetricKeyArnKmsMRKey):
            return False
        return self.value == other.value
...
KmsSymmetricKeyArn = Union[
    KmsSymmetricKeyArnKmsKeyArn, KmsSymmetricKeyArnKmsMRKey, KmsSymmetricKeyArnUnknown
]
...
def _kms_symmetric_key_arn_from_dict(d: Dict[str, Any]) -> KmsSymmetricKeyArn:
    ...
    if "KmsMRKey" in d:
        return KmsSymmetricKeyArnKmsMRKey.from_dict(d)
