...
def set_config_impl(config: Config):
    ...
    if isinstance(config, KeyStoreAdminConfig):
        from aws_cryptographic_material_providers.internaldafny.generated.KeyStoreAdmin import default__
        config.dafnyImplInterface.impl = default__.KeyStoreAdmin(smithy_config_to_dafny_config(config)).value
