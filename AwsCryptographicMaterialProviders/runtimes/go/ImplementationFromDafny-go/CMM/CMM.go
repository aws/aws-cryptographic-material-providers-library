// Package CMM
// Dafny module CMM compiled into Go
...
func (_static *CompanionStruct_Default___) RequiredEncryptionContextKeys_q(requiredEncryptionContextKeys m_Wrappers.Option, encryptionMaterials m_AwsCryptographyMaterialProvidersTypes.EncryptionMaterials) bool {
	return _dafny.Quantifier(((requiredEncryptionContextKeys).UnwrapOr(_dafny.SeqOf()).(_dafny.Sequence)).UniqueElements(), true, func(_forall_var_0 _dafny.Sequence) bool {
		var _0_k _dafny.Sequence
		_0_k = interface{}(_forall_var_0).(_dafny.Sequence)
		if m_UTF8.Companion_ValidUTF8Bytes_.Is_(_0_k) {
			return !(_dafny.Companion_Sequence_.Contains((requiredEncryptionContextKeys).UnwrapOr(_dafny.SeqOf()).(_dafny.Sequence), _0_k)) || (_dafny.Companion_Sequence_.Contains((encryptionMaterials).Dtor_requiredEncryptionContextKeys(), _0_k))
		} else {
			return true
		}
	})
...
var Companion_VerifiableInterface_ = CompanionStruct_VerifiableInterface_{
	TraitID_: &_dafny.TraitID{},
}

func (CompanionStruct_VerifiableInterface_) CastTo_(x interface{}) VerifiableInterface {
	var t VerifiableInterface
	t, _ = x.(VerifiableInterface)
	return t
}

// End of trait VerifiableInterface
