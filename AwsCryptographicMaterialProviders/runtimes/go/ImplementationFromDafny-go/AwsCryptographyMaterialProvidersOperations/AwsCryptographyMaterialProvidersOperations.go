...
func (_static *CompanionStruct_Default___) CheckCache(cache m_AwsCryptographyMaterialProvidersTypes.CacheType, ttlSeconds int64) m_Wrappers.Outcome {
	...
	if (cache).Is_StormTracking() {
		var _0_storm m_AwsCryptographyMaterialProvidersTypes.StormTrackingCache
		_ = _0_storm
		_0_storm = (cache).Dtor_StormTracking()
		if (ttlSeconds) <= (int64((_0_storm).Dtor_gracePeriod())) {
			var _1_msg _dafny.Sequence
			_ = _1_msg
			_1_msg = _dafny.Companion_Sequence_.Concatenate(_dafny.Companion_Sequence_.Concatenate(_dafny.Companion_Sequence_.Concatenate(_dafny.Companion_Sequence_.Concatenate(_dafny.Companion_Sequence_.Concatenate(_dafny.Companion_Sequence_.Concatenate(_dafny.SeqOfString("When creating an AwsKmsHierarchicalKeyring with a StormCache, "), _dafny.SeqOfString("the ttlSeconds of the KeyRing must be greater than the gracePeriod of the StormCache ")), _dafny.SeqOfString("yet the ttlSeconds is ")), Companion_Default___.N(ttlSeconds)), _dafny.SeqOfString(" and the gracePeriod is ")), Companion_Default___.N(int64((_0_storm).Dtor_gracePeriod()))), _dafny.SeqOfString("."))
			output = m_Wrappers.Companion_Outcome_.Create_Fail_(m_AwsCryptographyMaterialProvidersTypes.Companion_Error_.Create_AwsCryptographicMaterialProvidersException_(_1_msg))
			return output
		}
