...
/**
 * Converts cryptographic materials between the wire shapes and the MPL's types.
 *
 * <p>Materials travel on the wire rather than staying behind a handle, which is what
 * lets a test carry the output of one keyring's {@code OnEncrypt} into another
 * keyring's {@code OnDecrypt} -- eventually across two language servers. Nothing
 * server-side links the two halves of a round-trip.
 *
 * <p>The only asymmetry is the algorithm suite: the wire carries the identifier and the
 * MPL carries the full {@code AlgorithmSuiteInfo}, so the encode direction drops to the
 * id and the decode direction rehydrates through the MPL's own lookup. See
 * {@link AlgorithmSuites} for why that is lossless.
 */
public final class Materials {
    ...
    public static software.amazon.cryptography.materialproviders.model.EncryptionMaterials toMpl(
        ...
        var builder = software.amazon.cryptography.materialproviders.model.EncryptionMaterials.builder()
            .algorithmSuite(AlgorithmSuites.info(materialProviders, wire.getAlgorithmSuiteId()))
            .encryptionContext(orEmptyMap(wire.getEncryptionContext()))
            .encryptedDataKeys(edksToMpl(wire.getEncryptedDataKeys()))
            .requiredEncryptionContextKeys(orEmptyList(wire.getRequiredEncryptionContextKeys()));

        // Optional members are only set when present: the MPL distinguishes "absent"
        // from "empty" for these, and materials with a zero-length plaintext data key
        // are not the same as materials with none.
        ...
        if (wire.getSymmetricSigningKeys() != null) {
            builder.symmetricSigningKeys(new ArrayList<>(wire.getSymmetricSigningKeys()));
        }
    ...
    public static aws.cryptography.mpl.testserver.server.model.EncryptionMaterials toWire(
        ...
        var builder = aws.cryptography.mpl.testserver.server.model.EncryptionMaterials.builder()
            ...
            .requiredEncryptionContextKeys(orEmptyList(mpl.requiredEncryptionContextKeys()));
    ...
    public static software.amazon.cryptography.materialproviders.model.DecryptionMaterials toMpl(
        ...
        var builder = software.amazon.cryptography.materialproviders.model.DecryptionMaterials.builder()
            .algorithmSuite(AlgorithmSuites.info(materialProviders, wire.getAlgorithmSuiteId()))
            .encryptionContext(orEmptyMap(wire.getEncryptionContext()))
            .requiredEncryptionContextKeys(orEmptyList(wire.getRequiredEncryptionContextKeys()));
    ...
    public static aws.cryptography.mpl.testserver.server.model.DecryptionMaterials toWire(
        ...
        var builder = aws.cryptography.mpl.testserver.server.model.DecryptionMaterials.builder()
            .algorithmSuiteId(AlgorithmSuites.toWire(mpl.algorithmSuite().id()))
            .encryptionContext(orEmptyMap(mpl.encryptionContext()))
            .requiredEncryptionContextKeys(orEmptyList(mpl.requiredEncryptionContextKeys()));
    ...
    public static List<software.amazon.cryptography.materialproviders.model.EncryptedDataKey> edksToMpl(
        List<aws.cryptography.mpl.testserver.server.model.EncryptedDataKey> wire
    ) {
        if (wire == null) {
            return List.of();
        }
        List<software.amazon.cryptography.materialproviders.model.EncryptedDataKey> mpl =
            new ArrayList<>(wire.size());
        for (var edk : wire) {
            mpl.add(software.amazon.cryptography.materialproviders.model.EncryptedDataKey.builder()
    ...
    public static List<aws.cryptography.mpl.testserver.server.model.EncryptedDataKey> edksToWire(
        List<software.amazon.cryptography.materialproviders.model.EncryptedDataKey> mpl
    ) {
        if (mpl == null) {
            return List.of();
        ...
        for (var edk : mpl) {
            wire.add(aws.cryptography.mpl.testserver.server.model.EncryptedDataKey.builder()
    ...
    /**
     * Read a {@link ByteBuffer} without disturbing its position, so a buffer handed to
     * the MPL (or handed back) can still be read by the caller.
     *
     * <p>The MPL returns buffers it may itself still reference; consuming one in place
     * would corrupt a later read of the same materials.
     */
