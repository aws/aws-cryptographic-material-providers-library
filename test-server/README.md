...
| **A JDK 8–19** | to launch the MPL's *own* Gradle 7.6 build | The MPL pins a Java 8 toolchain, and Gradle 7.6 cannot run on a JDK newer than 19 — so the JDK above cannot launch it. Override with `MPL_JAVA_HOME=` |
...
2. **The version is read, never written.** `../project.properties` declares
   ...
   `1.11.1` is *not* a released version — only `1.11.0` is. Hard-coding any version
...
4. **A `$(shell)` hook can poison the Dafny command line.**
   ...
   *evaluates* it, so a publish prints a series of `BUILD FAILED in ~700ms` lines
...
- **`...tests`** — conformance tests. They validate *the MPL's* behavior: Raw AES
...
(as a *required* job), `push.yml`, and `daily_ci.yml`.
...
On failure the workflow uploads the JUnit reports from
`tests/build/{reports,test-results}/`.

## Known deviations and limitations

Reported here rather than papered over.

### Materials carry the algorithm suite *id*, not the full `AlgorithmSuiteInfo`
...
*properties* directly. The remedy for that, when a test needs it, is to expose
...
in its framework layer *before* any handler runs. An empty handle therefore surfaces
